...
async function uploadToGcs(file, folder) {
    ...
    const { uploadUrl, publicUrl, readUrl } = await apiFetch("/api/uploads/signed-url", {
        method: "POST",
    ...
    // Prefer the signed READ URL so the instant-preview <img src> works
    // against a private bucket. Fall back to publicUrl for older backends
    // that don't return readUrl yet. Either shape round-trips correctly
    // through toObjectPath on save (signed-URL query string is stripped).
...
// ─── Ken Burns Cover Slideshow ──────────────────────────

function getCoverPhotos(formOrItem) {
    const arr = Array.isArray(formOrItem.cover_photos) ? formOrItem.cover_photos.filter(Boolean) : [];
    if (arr.length > 0) return arr;
    if (formOrItem.cover_photo_url) return [formOrItem.cover_photo_url];
    return [];
}

const KB_DURATION = 5000; // ms per slide
const KB_TRANSITION = 1200; // crossfade ms

const KB_VARIANTS = [
    { transform: "scale(1.0)", transformEnd: "scale(1.12) translate(-1%, -1%)" },
    { transform: "scale(1.0)", transformEnd: "scale(1.12) translate(1%, -1%)" },
    { transform: "scale(1.12) translate(-1%, -1%)", transformEnd: "scale(1.0)" },
    { transform: "scale(1.12) translate(1%, 1%)", transformEnd: "scale(1.0)" },
];

function CoverSlideshow({ photos, coverPosition, height, children, onClick, sx: extraSx }) {
    ...
    // Track each image's assigned animation so it persists while fading out
    ...
    // When an image becomes active, assign it a new animation key
    useEffect(() => {
        animMapRef.current[activeIdx] = kbKey;
    }, [activeIdx, kbKey]);

    if (images.length === 0) return null;

    // Build keyframes for all images that have an assigned animation
    const allKeyframes = images.map((_, idx) => {
        const assignedKey = animMapRef.current[idx];
        if (assignedKey === undefined) return "";
        const variant = KB_VARIANTS[(assignedKey + idx) % KB_VARIANTS.length];
        return `@keyframes kb_${idx}_${assignedKey} { 0% { transform: ${variant.transform}; } 100% { transform: ${variant.transformEnd}; } }`;
    }).filter(Boolean).join("\n");
    ...
    return (
        ...
        >
            {images.map((src, idx) => {
                const assignedKey = animMapRef.current[idx] ?? kbKey;
                const variant = KB_VARIANTS[(assignedKey + idx) % KB_VARIANTS.length];
                ...
                return (
                    <Box
                        ...
                        sx={{
                            ...
                            animation: `kb_${idx}_${assignedKey} ${KB_DURATION}ms ease-in-out forwards`,
...
function CommunityPreview({ form, isEditing }) {
    ...
    const coverPhotos = Array.isArray(form.cover_photos) && form.cover_photos.length > 0 ? form.cover_photos : (form.cover_photo_url ? [form.cover_photo_url] : []);
    ...
    return (
        ...
        >
            {/* ══ Cover Photo ══ */}
            {hasCover ? (
                <CoverSlideshow photos={coverPhotos} coverPosition={form.cover_position} height={{ xs: 210, sm: 250 }}>
                    {form.tagline && (
                        <Box sx={{ position: "absolute", bottom: 0, left: 0, right: 0, p: 1.5, background: "linear-gradient(transparent, rgba(0,0,0,0.7))", zIndex: 2 }}>
                    ...
                    <Box sx={(t) => ({ borderRadius: 3, overflow: "hidden", border: "1px solid", borderColor: alpha(hlColor || t.palette.primary.main, 0.18), bgcolor: alpha(hlColor || t.palette.primary.main, 0.02) })}>
                        ...
                        {(sec.photo_url || sec.body) && (
                            <Box sx={{ overflow: "hidden" }}>
                                {sec.photo_url && (
                                    <Box component="img" src={sec.photo_url} alt={sec.title} sx={{ width: "100%", height: 405, objectFit: "cover", display: "block" }} />
                                )}
                                {sec.body && (
                                    <Box sx={{ px: 1.75, pt: sec.photo_url ? 1.25 : 1.75, pb: 1.75 }}>
                    ...
                    <Box sx={(t) => ({ borderRadius: 3, overflow: "hidden", border: "1px solid", borderColor: alpha(hlColor || t.palette.primary.main, 0.18), bgcolor: alpha(hlColor || t.palette.primary.main, 0.02) })}>
                        ...
                        <Box sx={{ overflow: "hidden" }}>
                            {form.highlight_photo_url && (
                                <Box component="img" src={form.highlight_photo_url} alt="" sx={{ width: "100%", height: 405, objectFit: "cover", display: "block" }} />
                            )}
                            {form.highlight_body && (
                                <Box sx={{ px: 1.75, pt: form.highlight_photo_url ? 1.25 : 1.75, pb: 1.75 }}>
            ...
            {/* ══ Organizer / Contact — matching CommunityDiscoverTab exactly ══ */}
            {hasOwner && (
                <>
                    ...
                    <Box sx={{ px: 2, pt: 2 }}>
                        ...
                        <Stack direction="row" spacing={2} alignItems="center">
                            {form.owner_avatar_url ? (
                                ...
                                <Avatar sx={{ width: 90, height: 90, borderRadius: 2.5, bgcolor: "grey.200" }}>
                                    <GroupsRoundedIcon sx={{ fontSize: 36, color: "grey.500" }} />
                                </Avatar>
                            )}
                            <Box sx={{ flex: 1, minWidth: 0 }}>
                                {form.owner_name && (
                                    <Typography sx={{ fontWeight: 900, fontSize: 16, lineHeight: 1.2 }}>{form.owner_name}</Typography>
                                )}
                                {form.owner_title && (
                        ...
                        <Fragment key={idx}>
                            <Stack direction="row" spacing={2} alignItems="center" sx={{ mt: 2 }}>
                                {ao.avatar_url ? (
                                    ...
                                    <Avatar sx={{ width: 90, height: 90, borderRadius: 2.5, bgcolor: "grey.200", fontSize: 18 }}>
                                        {(ao.name || "?")[0]}
                                    </Avatar>
                                )}
                                <Box sx={{ flex: 1, minWidth: 0 }}>
...
function BusinessPreview({ form, isEditing }) {
    ...
    return (
        ...
        >
            ...
            {hasCover ? (
                <CoverSlideshow photos={coverPhotos} coverPosition={form.cover_position} height={{ xs: 150, sm: 180 }}>
                    <Box sx={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(0,0,0,0.72) 0%, rgba(0,0,0,0.18) 50%, rgba(0,0,0,0.04) 100%)", zIndex: 1 }} />
                    <Box sx={{ position: "absolute", bottom: 0, left: 0, right: 0, px: 2, pb: 1.5, zIndex: 2 }}>
                        ...
                        {(sec.photo_url || sec.gif_url || sec.body) && (
                            <Box sx={{ overflow: "hidden" }}>
                                {(sec.photo_url || sec.gif_url) && (
                                    <Box component="img" src={sec.gif_url || sec.photo_url} alt={sec.title} sx={{ width: "100%", height: 405, objectFit: "cover", display: "block" }} />
                                )}
                                {sec.body && (
                                    <Box sx={{ px: 1.75, pt: (sec.photo_url || sec.gif_url) ? 1.25 : 1.75, pb: 1.75 }}>
...
function ArtistPreview({ form, isEditing }) {
    ...
    return (
        ...
        >
            ...
            {hasCover ? (
                <CoverSlideshow photos={coverPhotos} coverPosition={form.cover_position} height={{ xs: 180, sm: 220 }}>
                    <Box sx={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(0,0,0,0.78) 0%, rgba(0,0,0,0.25) 45%, rgba(0,0,0,0.05) 100%)", zIndex: 1 }} />
            ...
            <Box sx={{ px: 2, py: 1.25, display: "flex", alignItems: "center", gap: 1.25, borderBottom: "1px solid", borderColor: (t) => alpha(t.palette.divider, 0.5) }}>
                ...
                {/* Social icons — themed like ArtistCard */}
                <Stack direction="row" spacing={0.25} alignItems="center">
                    {form.facebook_url && (
                        <IconButton
                            component="a" href={form.facebook_url} target="_blank" rel="noopener noreferrer" size="small"
                            sx={(t) => {
                    ...
                    {form.instagram_url && (
                        <IconButton
                            component="a" href={form.instagram_url} target="_blank" rel="noopener noreferrer" size="small"
                            sx={(t) => {
                    ...
                    {form.tiktok_url && (
                        <IconButton
                            component="a" href={form.tiktok_url} target="_blank" rel="noopener noreferrer" size="small"
                            sx={(t) => {
                    ...
                    {form.website_url && (
                        <IconButton
                            component="a" href={form.website_url.startsWith("http") ? form.website_url : `https://${form.website_url}`} target="_blank" rel="noopener noreferrer" size="small"
                            sx={(t) => {
                    ...
                    {form.owner_email && (
                        <IconButton
                            component="a" href={`mailto:${form.owner_email}`} size="small"
                            sx={(t) => {
            ...
            {sections.length === 0 && hasLegacyHighlight && (
                <Box sx={{ px: 2, pt: 2 }}>
                    <Box sx={(t) => ({ borderRadius: 3, overflow: "hidden", border: "1px solid", borderColor: alpha(t.palette.divider, 0.6), bgcolor: alpha(t.palette.common.black, 0.02) })}>
                        ...
                        <Box sx={{ overflow: "hidden" }}>
                            {form.highlight_photo_url && (
                                <Box component="img" src={form.highlight_photo_url} alt={form.highlight_title} sx={{ width: "100%", height: 405, objectFit: "cover", display: "block" }} />
                            )}
                            <Box sx={{ px: 1.75, pt: form.highlight_photo_url ? 1.25 : 1.75, pb: 1.75 }}>
...
function ServicesPreview({ form, isEditing }) {
    ...
    return (
        ...
        >
            ...
            {/* ── Cover Photo Hero ── */}
            {hasCover && (
                <CoverSlideshow photos={coverPhotos} coverPosition={form.cover_position} height={{ xs: 150, sm: 170 }}>
                    <Box sx={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 50, background: "linear-gradient(to bottom, transparent, rgba(0,0,0,0.12))", zIndex: 1 }} />
                </CoverSlideshow>
            )}
                    ...
                    >
                        ...
                        {(sec.photo_url || sec.gif_url || sec.body) && (
                            <Box sx={{ overflow: "hidden" }}>
                                ...
                                {sec.body && (
                                    <Box sx={{ px: 1.75, pt: (sec.photo_url || sec.gif_url) ? 1.25 : 1.75, pb: 1.75 }}>
                                        <RichHtml html={sec.body} sx={{ fontSize: 13, lineHeight: 1.6, color: "text.secondary", fontWeight: 500 }} />
                                    </Box>
                                )}
                            </Box>
                        )}
                    ...
                    >
                        ...
                        {(form.highlight_photo_url || form.highlight_body) && (
                            <Box sx={{ overflow: "hidden" }}>
                                ...
                                {form.highlight_body && (
                                    <Box sx={{ px: 1.75, pt: form.highlight_photo_url ? 1.25 : 1.75, pb: 1.75 }}>
                                        <RichHtml html={form.highlight_body} sx={{ fontSize: 13, lineHeight: 1.6, color: "text.secondary", fontWeight: 500 }} />
                                    </Box>
                                )}
                            </Box>
                        )}
...
function HighlightSectionEditor({ section, index, onChange, onRemove, setSaveError }) {
    ...
    const handleCropComplete = async (croppedBlob) => {
        ...
        try {
            ...
            const url = await uploadToGcs(croppedFile, "discover-highlights");
            updateField("photo_url", url);
        } catch (err) {
            if (typeof setSaveError === "function") setSaveError(err.message || "Upload failed");
        } finally {
            setUploading(false);
        }
    };
    ...
    return (
        ...
        >
            ...
            <Stack spacing={2} sx={{ pt: 1 }}>
                ...
                {/* Section Image with crop */}
                <Box>
                    <Typography sx={{ fontWeight: 700, fontSize: 13, mb: 0.5, color: "text.primary" }}>
                        Section Image
                    </Typography>
                    <Typography variant="caption" sx={{ color: "text.secondary", display: "block", mb: 0.75 }}>
                        JPG, PNG, or WEBP — cropped to 16:9
                    </Typography>
                    {section.photo_url && (
                        <Box sx={{ mb: 1, position: "relative", display: "inline-block" }}>
                            <Box
                    ...
                    <Stack direction="row" spacing={1} alignItems="center">
                        <Button
                            ...
                            sx={{ textTransform: "none", fontWeight: 700, fontSize: 12, borderRadius: 2 }}
                        >
                            {uploading ? "Uploading\u2026" : section.photo_url ? "Replace" : "Upload"}
                        </Button>
                    </Stack>
                    <input ref={hlFileRef} type="file" accept="image/jpeg,image/png,image/webp" style={{ display: "none" }} onChange={handleFileSelect} />
                </Box>
            </Stack>
...
export default function DiscoverHighlightsAdmin() {
    ...
    const openEdit = (item) => {
        ...
        setForm({
            ...
            cover_photos:        Array.isArray(item.cover_photos) ? item.cover_photos : (item.cover_photo_url ? [item.cover_photo_url] : []),
        ...
        <Stack spacing={2}>
            ...
            <Stack direction={{ xs: "column", sm: "row" }} spacing={1} alignItems={{ xs: "stretch", sm: "center" }} justifyContent="space-between">
                ...
                <Button
                    ...
                    onClick={openCreate}
                    disabled={isEditing}
                    sx={{ textTransform: "none", fontWeight: 800, borderRadius: 999, alignSelf: { xs: "flex-start", sm: "auto" } }}
                >
                    New Highlight
                </Button>
            </Stack>
                    ...
                    <Stack spacing={2.5}>
                        ...
                        <Collapse in={expandedSections.media} timeout={300}>
                            <Stack spacing={2} sx={{ pt: 0.5 }}>
                                <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "110px 1fr" }, gap: 2, alignItems: "start" }}>
                                    ...
                                    <Box>
                                        ...
                                        >
                                            ...
                                            <input type="file" hidden accept="image/jpeg,image/png,image/webp,image/gif" onChange={async (e) => {
                                                const file = e.target.files?.[0];
                                                if (!file) return;
                                                try { const url = await uploadToGcs(file, "discover-highlights"); setForm((p) => ({ ...p, logo_url: url })); } catch (err) { setSaveError(err.message); }
                                                e.target.value = "";
                                            }} />
                                        </Box>
                                        {form.logo_url && (
                                    ...
                                    {/* Cover photos — multiple upload grid */}
                                    <Box>
                                        <Typography sx={{ fontSize: 12, fontWeight: 800, color: "text.secondary", mb: 0.75, textTransform: "uppercase", letterSpacing: 0.5 }}>
                                            Cover Photos
                                        </Typography>
                                        <Typography variant="caption" sx={{ color: "text.secondary", display: "block", mb: 1 }}>
                                            Upload multiple cover photos — they'll rotate as a Ken Burns slideshow. First image is also used as fallback.
                                        </Typography>

                                        {/* Existing photos grid */}
                                        {(() => {
                                            const allCovers = getCoverPhotos(form);
                                            return allCovers.length > 0 && (
                                                <Box sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))", gap: 1, mb: 1.5 }}>
                                                    {allCovers.map((url, idx) => (
                                                        <Box key={url + idx} sx={{ position: "relative", borderRadius: 2, overflow: "hidden", border: "1px solid", borderColor: "divider", aspectRatio: "16/10" }}>
                                                            <Box component="img" src={url} alt={`Cover ${idx + 1}`} sx={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                                                            <IconButton
                                                                size="small"
                                                                onClick={() => {
                                                                    setForm((p) => {
                                                                        const photos = (Array.isArray(p.cover_photos) ? [...p.cover_photos] : []).filter(Boolean);
                                                                        // If the deleted url is also in cover_photo_url, clear it
                                                                        const newCoverPhotoUrl = p.cover_photo_url === url ? "" : p.cover_photo_url;
                                                                        const newPhotos = photos.filter((u) => u !== url);
                                                                        return {
                                                                            ...p,
                                                                            cover_photos: newPhotos,
                                                                            cover_photo_url: newPhotos.length > 0 ? newPhotos[0] : newCoverPhotoUrl,
                                                                        };
                                                                    });
                                                                }}
                                                                sx={{ position: "absolute", top: 3, right: 3, width: 20, height: 20, bgcolor: "rgba(0,0,0,0.55)", color: "#fff", "&:hover": { bgcolor: "rgba(0,0,0,0.75)" } }}
                                                            >
                                                                <CloseIcon sx={{ fontSize: 12 }} />
                                                            </IconButton>
                                                            {idx === 0 && (
                                                                <Box sx={{ position: "absolute", bottom: 3, left: 3, bgcolor: "rgba(0,0,0,0.55)", borderRadius: 1, px: 0.5, py: 0.15 }}>
                                                                    <Typography sx={{ fontSize: 8, color: "white", fontWeight: 800, textTransform: "uppercase" }}>Primary</Typography>
                                                                </Box>
                                                            )}
                                                        </Box>
                                                    ))}
                                        ...
                                        <Box
                                            ...
                                            sx={(t) => ({
                                                display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
                                                width: "100%", minHeight: 80, borderRadius: 2.5, cursor: "pointer",
                                        ...
                                        >
                                            <Stack alignItems="center" spacing={0.5}>
                                                <AddPhotoAlternateRoundedIcon sx={{ fontSize: 24, color: "text.disabled" }} />
                                                <Typography sx={{ fontSize: 11, fontWeight: 700, color: "text.secondary" }}>
                                                    {getCoverPhotos(form).length > 0 ? "Add more cover photos" : "Add cover photos"}
                                                </Typography>
                                            </Stack>
                                            <input type="file" hidden multiple accept="image/jpeg,image/png,image/webp,image/gif" onChange={async (e) => {
                                                const files = Array.from(e.target.files || []);
                                                if (!files.length) return;
                                                for (const file of files) {
                                                    try {
                                                        const url = await uploadToGcs(file, "discover-highlights");
                                                        setForm((p) => {
                                                            const photos = [...(Array.isArray(p.cover_photos) ? p.cover_photos : []).filter(Boolean), url];
                                                            return { ...p, cover_photos: photos, cover_photo_url: photos[0] || url };
                                                        });
                                        ...
                                        {/* Position selector (applies to all covers) */}
                                        {getCoverPhotos(form).length > 0 && (
                                            <Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 1 }}>
                                                <TextField select size="small" value={form.cover_position} onChange={setField("cover_position")} label="Position" sx={{ flex: 1, "& .MuiInputBase-input": { fontSize: 11, py: 0.5, px: 1 } }}>
                                                    <MenuItem value="top">Top</MenuItem>
                                                ...
                                                <Tooltip title="Remove all covers" arrow>
                                                    <IconButton size="small" onClick={() => setForm((p) => ({ ...p, cover_photo_url: "", cover_photos: [] }))} sx={{ width: 24, height: 24 }}>
                                ...
                                {form.video_id && (
                                    <MediaUploadField label="Video Thumbnail" helperText="Custom thumbnail shown before video plays" value={form.video_thumbnail_url} onChange={setPhotoField("video_thumbnail_url")} folder="discover-highlights" />
                                )}
                            </Stack>
                        </Collapse>
                        ...
                        <Collapse in={expandedSections.organizer} timeout={300}>
                            <Stack spacing={2} sx={{ pt: 0.5 }}>
                                ...
                                <MediaUploadField label={isArtist ? "Secondary Photo" : isCommunity ? "Organizer Photo" : "Owner Photo"} value={form.owner_avatar_url} onChange={setPhotoField("owner_avatar_url")} folder="discover-highlights" acceptGif />
                                {form.owner_avatar_url && (
                                        ...
                                        {(form.additional_owners || []).map((ao, aoIdx) => (
                                            <Paper key={aoIdx} variant="outlined" sx={{ p: 1.5, pt: 2, borderRadius: 2, position: "relative" }}>
                                                ...
                                                <Stack spacing={1}>
                                                    ...
                                                    <MediaUploadField label="Photo" value={ao.avatar_url || ""} onChange={(url) => setForm((p) => { const arr = [...p.additional_owners]; arr[aoIdx] = { ...arr[aoIdx], avatar_url: url }; return { ...p, additional_owners: arr }; })} folder="discover-highlights" />
