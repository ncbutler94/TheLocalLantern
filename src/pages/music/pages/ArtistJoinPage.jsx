// src/pages/music/pages/ArtistJoinPage.jsx
/**
 * ArtistJoinPage
 * --------------
 * Route: /music/artists/invite/:token
 *
 * Handles the recipient side of an artist team invite (email or link).
 *
 * Flow:
 *  1. Validate token via GET /api/music/artists/invite/:token/validate
 *  2. Show artist info + "Join Team" button
 *  3. If not logged in → redirect to login with return URL
 *  4. If on a non-personal account → prompt to switch
 *  5. On accept → POST /api/music/artists/invite/:token/accept
 *  6. On success → navigate to artist admin console
 */
...
export default function ArtistJoinPage() {
    ...
    const [joinedArtist, setJoinedArtist] = useState(null);
        ...
        (async () => {
            try {
                const res = await fetch(
                    apiUrl(`/api/music/artists/invite/${encodeURIComponent(token)}/validate`),
                    {
                        method: 'GET',
                        credentials: 'include',
                ...
                if (!res.ok) {
                    const errMsg = data?.error || data?.message || 'Unable to validate invite.';
                    setError(errMsg);
                    setStatus('error');
                    return;
                }
                ...
                if (!data.user_authenticated) {
                    const returnTo = `/music/artists/invite/${encodeURIComponent(token)}`;
                    try {
    ...
    const handleAccept = async () => {
        ...
        try {
            const res = await fetch(
                apiUrl(`/api/music/artists/invite/${encodeURIComponent(token)}/accept`),
                {
                    method: 'POST',
                    credentials: 'include',
                    headers: { Accept: 'application/json' },
            ...
            setJoinedArtist({
                id: data.artistId || invite?.artist_id,
                name: data.artistName || invite?.artist_name,
                handle: data.artistHandle || invite?.artist_handle,
            });
            setStatus('success');
        } catch (e) {
    ...
    const handleGoToAdmin = () => {
        const id = joinedArtist?.id;
        if (id) {
            navigate(`/music/artists/${id}/admin`);
        } else {
            navigate('/');
        }
    };
    ...
    // ---- Render ----
    const artistName = invite?.artist_name || 'this artist';
    const artistAvatar = invite?.artist_avatar || defaultAvatar;
                        ...
                        {status === 'ready' ? (
                            <>
                                ...
                                <Typography variant="body2" sx={{ color: 'text.secondary', mb: 2.5, lineHeight: 1.6 }}>
                                    ...
                                    <strong>{artistName}</strong>. You'll be able to manage the artist profile,
                        ...
                        {status === 'success' ? (
                            <Box sx={{ textAlign: 'center', py: 1 }}>
                                ...
                                <Typography variant="body2" sx={{ color: 'text.secondary', mb: 3, lineHeight: 1.6 }}>
                                    You've joined <strong>{joinedArtist?.name || artistName}</strong> as an admin.
                                    You can now manage the artist profile from the admin console.
                                </Typography>
                                <Button
                                    variant="contained"
                                    fullWidth
