...
/** Build profile path – always /{handle} regardless of account type */
...
function getPostDetailPath(p) {
    ...
    if (cat === 'business_post') {
        ...
        if (bizId) return `/${encodeURIComponent(bizId)}/posts/${id}`;
        return `/posts/${id}`;
    }

    if (cat === 'artist_post') {
        const handle = p?.artistHandle || p?.artist_handle || p?.handle || '';
        if (handle) return `/${encodeURIComponent(handle)}/posts/${id}`;
        const artId = p?.artistId || p?.artist_id || p?.music_artist_id || '';
        if (artId) return `/${encodeURIComponent(artId)}/posts/${id}`;
        return `/posts/${id}`;
    }
...
/** Reusable row for category Select items — icon + label + optional count, matching CommunityFilter style */
function CategoryRow({ icon: IconComp, label, count }) {
            ...
            {hasCount && (
                <Typography
                    ...
                    sx={(t) => ({
                        ...
                        bgcolor: count > 0 ? alpha(t.palette.primary.main, 0.1) : alpha(t.palette.action.hover, 0.6),
                        px: 0.75,
                        py: 0.15,
                        borderRadius: 1,
                        minWidth: 24,
...
const COUNTRY_NAMES = {
    ...
    IE: 'Ireland', NZ: 'New Zealand', IN: 'India', MX: 'Mexico', BR: 'Brazil',
...
/** Pill-shaped frosted-glass FormControl styling — matches CommunityFilter */
...
/** Same frosted style but for CityCountySelect */
const CREAM_INPUT_SX = Object.freeze({ ...CONTROL_SX });
...
function UserCard({
    ...
    const computedLocationText = (() => {
        ...
        const countyRaw = user?.home_county || user?.county || '';
        const state = String(user?.home_state || user?.state || '').trim().toLowerCase();
        const isAlabama = !state || state === 'al' || state === 'alabama';
        // For Alabama users, append "County" to the county name on the card
        const county = countyRaw && isAlabama
            ? `${normalizeCountyName(countyRaw)} County`
            : countyRaw;
    ...
    const hasMutuals = Array.isArray(mutualFollowers) && mutualFollowers.length > 0;

    return (
        <Paper
            variant="outlined"
            sx={{
                p: compact ? 0.75 : { xs: 1.25, sm: 1.5 },
                borderRadius: compact ? 2 : { xs: 0, sm: 3 },
                width: '100%',
                height: compact ? undefined : { xs: 'auto', sm: 110 },
                minHeight: compact ? undefined : { xs: 64, sm: 110 },
                display: 'flex',
                flexDirection: 'column',
                justifyContent: 'center',
                bgcolor: 'background.paper',
                borderColor: (t) => alpha(t.palette.divider, 0.15),
                boxShadow: compact ? 'none' : { xs: 'none', sm: ((t) => `0 10px 26px ${alpha(t.palette.text.primary, 0.06)}`) },
                ...
                '&:hover': {
                    transform: 'none',
                    boxShadow: { xs: 'none', sm: ((t) => `0 14px 34px ${alpha(t.palette.text.primary, 0.08)}`) },
        ...
        >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: compact ? 0.75 : 1.5 }}>
                {(() => {
                    ...
                    // Distinguish musicians (music note) from visual artists (palette)
                    // using profile_type. Defaults to music-note when missing.
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: compact ? 0.5 : 1, flexShrink: 0 }}>
                    {actionLabel ? (() => {
                        const isFollowing = actionVariant === 'outlined';

                        // Mobile: compact icon button
                        if (isMobileCard && !compact) {
                            return (
                                <IconButton
                                    size="small"
                                    ...
                                    sx={(t) => ({
                                        ...
                                        transition: `all ${t.custom?.motion?.fast || 150}ms ease`,
                                    })}
                                >
                                    {isFollowing
                        ...
                        // Desktop / compact: full button (unchanged)
                        return (
                            <Button
                                size="small"
                    ...
                    {!hideMenu ? (
                        <IconButton size="small" onClick={(e) => { e.stopPropagation(); onOpenUserCard(e.currentTarget, user); }}>
                ...
                >
                    <Box sx={{ display: 'flex', flexShrink: 0 }}>
                        {mutualFollowers.slice(0, 3).map((mf, idx) => {
                            const mfAvatar = mf?.avatar_url || mf?.profile_picture || '';
                            const mfShowIcon = !mfAvatar;
                            return (
                                <Avatar
                                    key={mf.id || idx}
                                    src={mfShowIcon ? undefined : mfAvatar}
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        if (typeof onOpenUserCard === 'function') onOpenUserCard(e.currentTarget, mf);
                                    }}
                                    sx={(t) => ({
                                        width: 20,
                                        height: 20,
                                ...
                                >
                                    {mfShowIcon ? <PersonRoundedIcon sx={{ fontSize: 13 }} /> : null}
                                </Avatar>
                            );
                        })}
                    </Box>
                    ...
                    >
                        Followed by{' '}
                        {mutualFollowers.slice(0, 2).map((mf, idx) => {
                            const mfAcctType = getAcctType(mf);
                            const isNonPersonal = mfAcctType === 'business' || mfAcctType === 'artist';
                            const rawName = String(
                                mf?.business_name || mf?.businessName ||
                                mf?.pageName || mf?.page_name ||
                                mf?.first_name || 'Someone'
                            ).trim();
                            const firstName = isNonPersonal ? rawName : rawName.split(' ')[0];
                            return (
                                <React.Fragment key={mf.id || idx}>
                                    {idx > 0 && (mutualFollowers.length > 2 ? ', ' : ' & ')}
                                    <Box
                                        component="span"
                                        onClick={(e) => {
                                            e.stopPropagation();
                        ...
                        {mutualFollowers.length > 2 && (
                            <Box component="span"> +{mutualFollowers.length - 2} more</Box>
                        )}
                        {' '}you follow
                    </Typography>
                </Box>
...
function TabIconWrapper({ children, size = 22, mobileSize, squeezeX = 1 }) {
    const sx = Number(squeezeX) || 1;
    const fontSize = typeof size === 'number' ? size : 22;
    const mobileFontSize = typeof mobileSize === 'number' ? mobileSize : Math.max(fontSize - 6, 14);
        ...
        >
            {React.cloneElement(children, {
                ...
                sx: {
                    ...
                    fontSize: { xs: mobileFontSize, sm: fontSize },
                },
            })}
        </Box>
    );
...
export default function SocialHome({ me }) {
    ...
    // chromeTop is the height of the global app bar; used for fit calcs below.
    // The page shell uses `--ll-nav-offset` (a CSS variable set continuously
    // by Header.jsx during scroll-hide, 0..1) to expand and cover the space
    // vacated by the app bar + bottom nav as they slide away — so no JS
    // measurement of the page's own height is needed.
    const [chromeTop, setChromeTop] = useState(0);

    // Measure the header height so we can pull the page up when the header hides
    useLayoutEffect(() => {
        const measure = () => {
            const header =
                ...
                null;
            setChromeTop(header ? header.getBoundingClientRect().height : 0);
        };
        measure();
    ...
    const [tab, setTab] = useState(() => {
        ...
        // New: 0 Feed, 1 People, 2 Following, 3 Followers, 4 Requests, 5 Safety
        try {
    ...
    useEffect(() => {
        ...
        if (intent === 'followers') {
            ...
            const sortHint = String(location?.state?.sortFollowersBy || '').trim().toLowerCase();
            if (sortHint === 'recent') setFollowersSortBy('recent');
            initialTabAppliedRef.current = true;
            return;
        }
        ...
        if ((intent === 'requests' || intent === 'follow-requests' || intent === 'followrequests') && !isBusinessAccount && !isArtistAccount) {
            setTab(TAB.REQUESTS);
            initialTabAppliedRef.current = true;
            return;
        }
        ...
        setFeedTab(0);
        initialTabAppliedRef.current = true;
    }, [meResolved, isLoggedIn, isBusinessAccount, isArtistAccount, location.key, location.search, TAB.FEED, TAB.PEOPLE]);
    ...
    useEffect(() => {
        ...
        if (intent === 'people' || intent === 'find-people' || intent === 'findpeople') {
            ...
            setTab(TAB.FOLLOWERS);
            // If the caller requested a specific sort (e.g. arriving from a
            // grouped follow notification), honor it; otherwise reset to
            // default so normal navigation doesn't carry over a stale sort.
            const sortHint = String(location?.state?.sortFollowersBy || '').trim().toLowerCase();
            setFollowersSortBy(sortHint === 'recent' ? 'recent' : 'default');
        } else if (intent === 'following') {
            setTab(TAB.FOLLOWING);
        } else if (intent === 'safety') {
            setTab(TAB.SAFETY);
        } else if ((intent === 'requests' || intent === 'follow-requests' || intent === 'followrequests') && !isBusinessAccount && !isArtistAccount) {
            setTab(TAB.REQUESTS);
        }
...
// People filters (wait for Search button)
    ...
    const [peopleSearchApplied, setPeopleSearchApplied] = useState('');
    ...
    // Out-of-state location filters for People tab (country + US state)
    const [peopleOosCountry, setPeopleOosCountry] = useState('');
    const [peopleOosState, setPeopleOosState] = useState('');
    ...
    // Sort mode for Followers tab — 'default' keeps the server order,
    // 'recent' sorts by most recently followed (newest first). Set via
    // navigation state when arriving from a grouped follow notification.
    const [followersSortBy, setFollowersSortBy] = useState('default');

    // Location counts for Find People filter badges
    const [peopleLocationCounts, setPeopleLocationCounts] = useState({ counties: {}, cities: {} });

    // Mutual followers map: { targetUserId: [ { id, first_name, last_name, handle, avatar_url }, … ] }
    const [mutualFollowersMap, setMutualFollowersMap] = useState({});
    ...
    // Optimistic unfollow tracking — used to instantly hide unfollowed users
    const [locallyUnfollowed, setLocallyUnfollowed] = useState(() => new Set());
    ...
    const [followRequestsSeen, setFollowRequestsSeen] = useState(false);
    const viewerIsPrivate = Boolean(viewer?.is_private || viewer?.isPrivateAccount);
    const { showSuccess: showRequestSnack, snackbarProps: requestSnackProps } = useSuccessSnackbar();
...
// Feed data (from people you follow)
    ...
    const [mobilePeopleFilterOpen, setMobilePeopleFilterOpen] = useState(false);
    const [mobileFollowFilterOpen, setMobileFollowFilterOpen] = useState(false);
    const [showPeopleFilters, setShowPeopleFilters] = useState(true);

    // ── Report confirmation dialog (matches detail panel style) ──
    const [reportConfirmOpen, setReportConfirmOpen] = useState(false);
    const handleReportSuccess = useCallback(() => { setReportConfirmOpen(true); }, []);

    // ── Continuous subheader scroll-hide (Facebook-style) ──
    // Three chrome elements slide with scroll on mobile:
    //   1. toolbarHeaderRef — the contextual toolbar Paper (search/filter row
    //      above People/Following/Followers list; also wraps the guest login
    //      banner so it hides with the toolbar).
    //   2. profileSidebarRef — the left-column profile block on mobile
    //      (avatar, tabs counts, etc.).
    //   3. feedTabsRef — the inline tabs row inside the Feed view.
    //
    // Two possible scroll targets — only one is active at a time depending on
    // which outer tab the user is on:
    //   [data-social-people-scroll] for PEOPLE / FOLLOWING / FOLLOWERS tabs
    //   [data-social-feed-scroll]   for FEED tab
    //
    // Each ref gets two hook calls; only the one matching the current tab is
    // enabled. The hook cleanly detaches/attaches as `enabled` flips, so there
    // are no stray listeners.
    // ── Mobile subheader fade (replaces translate-based scroll-hide) ──
    // Previously these hooks translateY'd the toolbar/sidebar/feed-tabs refs
    // and reclaimed their vertical space via negative margin-bottom as the
    // user scrolled. That produced jerky content shifts as the bars moved
    // and the feed pulled up behind them. The three refs are now
    // `position: sticky` under the global header and fade via
    // `opacity: calc(1 - var(--ll-nav-offset))`. Same CSS var as Header.jsx —
    // every piece of chrome fades in lockstep. Hooks disabled (not removed)
    // in case we want to reintroduce hybrid motion later.
    const toolbarHeaderRef = useRef(null);
    const profileSidebarRef = useRef(null);
    const feedTabsRef = useRef(null);

    const onPeopleScrollTab = tab === TAB.PEOPLE || tab === TAB.FOLLOWING || tab === TAB.FOLLOWERS;
    const onFeedScrollTab = tab === TAB.FEED;

    // Toolbar header — previously active on People-like tabs
    useSubheaderScrollHide({
        headerRef: toolbarHeaderRef,
        scrollTargetSelector: '[data-social-people-scroll]',
        enabled: false,
    });
    // Profile sidebar — previously active on whichever tab is currently scrolling
    useSubheaderScrollHide({
        headerRef: profileSidebarRef,
        scrollTargetSelector: '[data-social-people-scroll]',
        enabled: false,
    });
    useSubheaderScrollHide({
        headerRef: profileSidebarRef,
        scrollTargetSelector: '[data-social-feed-scroll]',
        enabled: false,
    });
    // Feed tabs row — previously active on Feed tab
    useSubheaderScrollHide({
        headerRef: feedTabsRef,
        scrollTargetSelector: '[data-social-feed-scroll]',
        enabled: false,
    });

    // ── Write the live profile-sidebar height to --ll-subheader-height ──
    // On mobile the profile sidebar is the floating chrome below the global
    // header. The scroll containers reserve space for it via `padding-top:
    // calc(header + subheader)` so content doesn't sit under the chrome on
    // initial paint. ResizeObserver keeps the CSS var in sync with the real
    // height. Mobile only.
    useEffect(() => {
        if (!isMobile) {
        ...
        const el = profileSidebarRef.current;
    ...
    const followingFiltered = useMemo(() => {
        // First, exclude anyone the user just unfollowed (optimistic removal)
        const base = (following || []).filter((u) => {
            const id = Number(u?.id);
            return !locallyUnfollowed.has(id);
        });
        ...
        if (!term && !countySel && !citySel && !followingOutOfState && !followingAccountType) return base;
    ...
    }, [following, followingSearchApplied, followingCountyApplied, followingCityApplied, followingOutOfState, followingAccountType, publicProfileById, locallyUnfollowed]);
    ...
    const followersFiltered = useMemo(() => {
        ...
        // Helper: sort by followed_at DESC (most recent first). Users whose
        // followed_at is null/unknown (legacy follows) sort to the bottom.
        const sortByRecent = (list) => {
            const arr = Array.isArray(list) ? list.slice() : [];
            arr.sort((a, b) => {
                const aRaw = a?.followed_at;
                const bRaw = b?.followed_at;
                ...
                if (aHas && bHas) return bMs - aMs;
                if (aHas) return -1;   // a has a timestamp, b doesn't → a first
                if (bHas) return 1;    // b has a timestamp, a doesn't → b first
                return 0;              // both unknown — preserve input order
            });
            return arr;
        };

        const applySort = (list) =>
            followersSortBy === 'recent' ? sortByRecent(list) : list;

        if (!term && !countySel && !citySel && !followersOutOfState && !followersAccountType) {
            return applySort(followers);
        }
    ...
    }, [followers, followersSearchApplied, followersCountyApplied, followersCityApplied, followersOutOfState, followersAccountType, followersSortBy, publicProfileById]);

    // Client-side out-of-state filter for People tab (rows come from server)
    const peopleFiltered = useMemo(() => {
        // Server-side filtering handles out-of-state, country, and state now.
        // Client-side filtering is no longer needed for out-of-state.
        return rows;
    }, [rows]);

    // Out-of-state location counts fetched from the server
    const [peopleOosLocationCounts, setPeopleOosLocationCounts] = useState({ countries: {}, states: {} });

    // Reset render count when out-of-state filters change (triggers new server fetch)
    useEffect(() => {
        const filtered = Array.isArray(peopleFiltered) ? peopleFiltered.length : 0;
        setPeopleRenderCount(Math.max(PAGE_SIZE, filtered));
    }, [peopleOutOfState, peopleOosCountry, peopleOosState]);
    ...
    // Quick unfollow — no confirmation needed
...
// Local optimistic flips
    ...
    const [locallyRequested, setLocallyRequested] = useState(() => new Set());
...
// Hydrate target from /users/public/:handleOrId to resolve numeric id and current follow state
    ...
    const isFollowingForCard = useMemo(() => {
        ...
        if (!tid) return false;
        if (locallyUnfollowed.has(tid)) return false;
        return serverFollowingSet.has(tid) || locallyFollowed.has(tid);
    }, [userForCard, serverFollowingSet, locallyFollowed, locallyUnfollowed]);
    ...
    const postFollow = async (targetId, action, targetType = 'user') => {
        ...
        const urls = isScoped
            ...
            : ['/api/follows/toggle', `${api}/api/follows/toggle`, `${api}/users/follow`, '/api/users/follow', '/users/follow'];
        try {
            const resp = await tryPost(urls, payload, { withCredentials: true });
    ...
    const handleFollowToggle = async (targetUser, forceAction) => {
        ...
        requireAuth(async () => {
            ...
            const currentlyFollowing = (serverFollowingSet.has(tid) || locallyFollowed.has(tid)) && !locallyUnfollowed.has(tid);
            ...
            if (action === 'follow') {
                ...
                setLocallyUnfollowed((prev) => { const next = new Set(prev); next.delete(tid); return next; });
            } else {
                setLocallyFollowed((prev) => { const next = new Set(prev); next.delete(tid); return next; });
                setLocallyRequested((prev) => { const next = new Set(prev); next.delete(tid); return next; });
                setLocallyUnfollowed((prev) => { const next = new Set(prev); next.add(tid); return next; });
            }

            const targetType = String(targetUser?.account_type || 'user').toLowerCase();
            const result = await postFollow(tid, action, targetType);
            if (result) {
                if (result.requested && !result.following) {
                    setLocallyFollowed((prev) => { const next = new Set(prev); next.delete(tid); return next; });
                    setLocallyRequested((prev) => { const next = new Set(prev); next.add(tid); return next; });
                    setLocallyUnfollowed((prev) => { const next = new Set(prev); next.delete(tid); return next; });
                    showRequestSnack(`Follow request sent to ${targetUser?.first_name || targetUser?.handle || 'user'}.`);
                } else {
                    setServerFollowingSet((prev) => {
                        const next = new Set(prev);
                        if (action === 'follow') next.add(tid); else next.delete(tid);
                        return next;
                    });
                    // Clear optimistic state now that server confirmed
                    if (action === 'unfollow') {
                        setLocallyUnfollowed((prev) => { const next = new Set(prev); next.delete(tid); return next; });
                    } else {
                        setLocallyFollowed((prev) => { const next = new Set(prev); next.delete(tid); return next; });
                ...
                setLocallyFollowed((prev) => {
                    const next = new Set(prev);
                    if (currentlyFollowing) next.add(tid); else next.delete(tid);
                    return next;
                });
                setLocallyUnfollowed((prev) => {
                    const next = new Set(prev);
                    if (currentlyFollowing) next.delete(tid); else next.add(tid);
                    return next;
                });
            }
        });
    ...
    const handleAcceptRequest = useCallback(async (requestId) => {
        ...
        const name = req ? `${req.first_name || ''} ${req.last_name || ''}`.trim() || 'User' : 'User';
        try {
            await tryPost(
                [`/users/follow-requests/${requestId}/accept`, `${api}/users/follow-requests/${requestId}/accept`, `/api/users/follow-requests/${requestId}/accept`, `${api}/api/users/follow-requests/${requestId}/accept`],
                {},
                { withCredentials: true }
            );
            setFollowRequests((prev) => prev.filter((r) => r.id !== requestId));
            setFollowRequestsCount((prev) => Math.max(0, prev - 1));
            showRequestSnack(`${name} can now follow you`);
            fetchSocial();
        } catch { /* ignore */ }
    }, [fetchSocial, followRequests, showRequestSnack]);
    ...
    const handleDeclineRequest = useCallback(async (requestId) => {
        ...
        try {
            await tryPost(
                [`/users/follow-requests/${requestId}/decline`, `${api}/users/follow-requests/${requestId}/decline`, `/api/users/follow-requests/${requestId}/decline`, `${api}/api/users/follow-requests/${requestId}/decline`],
                {},
                { withCredentials: true }
            );
            setFollowRequests((prev) => prev.filter((r) => r.id !== requestId));
            setFollowRequestsCount((prev) => Math.max(0, prev - 1));
            showRequestSnack(`Request from ${name} declined`);
        } catch { /* ignore */ }
    }, [followRequests, showRequestSnack]);

    const fetchPeople = useCallback(
        async (overrides = {}) => {
            const reset = overrides.reset !== undefined ? overrides.reset : !overrides.append;
            const effectiveSearch =
                overrides.search !== undefined ? overrides.search : peopleSearchApplied;
            ...
            if (reset) {
                setLoadingPeople(true); loadingPeopleRef.current = true;
                setLoadingMorePeople(false); loadingMorePeopleRef.current = false;
            } else {
                setLoadingMorePeople(true); loadingMorePeopleRef.current = true;
            }
            ...
            try {
                ...
                // When out-of-state is active, use server-side filtering instead of county/city
                if (peopleOutOfState) {
                    userQs.set('outOfState', '1');
                    if (peopleOosCountry) userQs.set('country', peopleOosCountry);
                    if (peopleOosState) userQs.set('state', peopleOosState);
                } else {
                ...
                // Business and artist results are intentionally excluded from
                // Find People — this tab only shows personal user accounts.
                const bizRows = [];
                const artistRows = [];

                const userResult = await userFetch;

                // Parse user result — only keep personal accounts for Find People
                const j = userResult;
                ...
                if (reset) {
                    ...
                    setPeopleHasMore(nextUsers.length < nextTotal); peopleHasMoreRef.current = nextUsers.length < nextTotal;
                    setBizOffset(bizRows.length); setBizHasMore(bizRows.length >= limit); bizHasMoreRef.current = bizRows.length >= limit;
                    setArtOffset(artistRows.length); setArtHasMore(artistRows.length >= limit); artHasMoreRef.current = artistRows.length >= limit;
                    ...
                    setPeopleHasMore(offset + nextUsers.length < nextTotal); peopleHasMoreRef.current = offset + nextUsers.length < nextTotal;
                    // Expand render count to show the newly fetched rows
                    setPeopleRenderCount((c) => c + nextUsers.length);
                }
            } catch (err) {
                ...
                if (reset) {
                    ...
                    setPeopleTotal(0);
                    setPeopleOffset(0);
                    setPeopleHasMore(false); peopleHasMoreRef.current = false;
                    setPeopleRenderCount(PAGE_SIZE);
                }
            } finally {
                setLoadingPeople(false); loadingPeopleRef.current = false;
                setLoadingMorePeople(false); loadingMorePeopleRef.current = false;
            }
        },
        [peopleSearchApplied, peopleCounty, peopleCity, peopleOffset, peopleTotal, peopleOutOfState, peopleOosCountry, peopleOosState]
    );

    // (Business/Artist scroll pagination removed — Find People only shows personal users)
        ...
        try {
            ...
            try {
                ...
                artPosts.forEach((ap) => {
                    ...
                    const ownerId = Number(ap?.userId || ap?.user_id || ap?.owner_user_id || ap?.createdByUserId || ap?.created_by_user_id || 0);
                ...
                // Resolve missing owner_user_id for artist identities by fetching the
                // artist profile. Without a valid userId the user-engagement endpoint
                // will 404, so this lookup is essential for artist comment data.
                const artMissingOwner = bizArtIdentities.filter((i) => i.accountType === 'artist' && !i.userId && i.artistId > 0);
                if (artMissingOwner.length) {
                    await Promise.allSettled(artMissingOwner.map(async (identity) => {
                        const artId = identity.artistId;
                        const lookupUrls = [
                            `${api}/api/music/artists/${artId}`,
                            `/api/music/artists/${artId}`,
                        ];
                        for (const u of lookupUrls) {
                            try {
                                const res = await axios.get(u, { withCredentials: true, timeout: 6000 });
                                const d = res?.data || {};
                                const owId = Number(d?.owner_user_id || d?.ownerUserId || d?.user_id || d?.userId || 0);
                                if (owId > 0) { identity.userId = owId; }
                                // Backfill profileType from the artist record if we don't have one yet
                                if (!identity.profileType) {
                                    const pt = String(d?.profile_type || d?.profileType || '').toLowerCase();
                                    if (pt) identity.profileType = pt;
                                }
                                if (identity.userId) break;
                            } catch { /* try next */ }
                        }
                    }));
                }
            ...
            // Helper: fetch from the first URL that succeeds
            const tryGetJson = async (urls, timeout = 12000) => {
                for (const u of urls.filter(Boolean)) {
                    try {
                        const res = await axios.get(u, { withCredentials: true, timeout });
                        return res?.data || {};
                    } catch { /* try next */ }
                }
                return {};
            ...
            // ── Load engagement for a personal or artist identity via the
            //    unified /users/:id/engagement/posts endpoint ──
            const loadUserEngagement = async (identity) => {
                const { userId, artistId } = identity;
                if (!userId) return {};
            ...
            // ── Load engagement for a business identity via the dedicated
            //    /api/business/:id/activity/* endpoints (no owner_user_id needed) ──
            const loadBusinessEngagement = async (identity) => {
                const { businessId } = identity;
                if (!businessId) return {};

                const [commentsRes, likesRes, repostsRes] = await Promise.allSettled([
                    tryGetJson([
                        `${api}/api/business/${businessId}/activity/comments`,
                        `/api/business/${businessId}/activity/comments`,
                    ]),
                    tryGetJson([
                        `${api}/api/business/${businessId}/activity/likes`,
                        `/api/business/${businessId}/activity/likes`,
                    ]),
                    tryGetJson([
                        `${api}/api/business/${businessId}/activity/reposts`,
                        `/api/business/${businessId}/activity/reposts`,
                    ]),
                ]);

                // Flatten grouped comments: business activity returns
                // { comments: [{ post: {...}, comments: [{...}] }] }
                // We need flat comment rows with embedded post objects.
                const rawGroups = Array.isArray((commentsRes.status === 'fulfilled' ? commentsRes.value : {})?.comments)
                    ? commentsRes.value.comments : [];
                const flatComments = [];
                rawGroups.forEach((group) => {
                    ...
                    cArr.forEach((c) => {
                        flatComments.push({
                            ...(c || {}),
                            id: c?.id || c?.comment_id || 0,
                            comment_id: c?.id || c?.comment_id || 0,
                            post_id: Number(post?.id || c?.post_id || 0),
                ...
                // Likes/reposts from business activity come as { items: [...] }
                const rawLikes = Array.isArray((likesRes.status === 'fulfilled' ? likesRes.value : {})?.items)
                    ? likesRes.value.items : [];
                const rawReposts = Array.isArray((repostsRes.status === 'fulfilled' ? repostsRes.value : {})?.items)
                    ? repostsRes.value.items : [];

                return { comments: flatComments, likes: rawLikes, reposts: rawReposts };
            };

            const loadEngagementForIdentity = async (identity) => {
                const { accountType, businessId } = identity;
                if (accountType === 'business' && businessId > 0) {
                    return loadBusinessEngagement(identity);
                }
                return loadUserEngagement(identity);
            };
            ...
            [...likesAgg, ...repostsAgg, ...commentsAgg].forEach((item) => {
                ...
                if (acctType === 'business' || acctType === 'artist') {
                    ...
                    actorSeeds[compositeKey] = {
                        ...
                        profile_type: item._actor_profile_type || '',
                    };
    ...
    const fetchModeration = useCallback(async () => {
        ...
        try {
            ...
            // ── Unified: moderation state is shared across all accounts,
            // no need to pass account context ──
            const res = await tryGet(
                [`${api}/users/moderation-state`, `/users/moderation-state`, `/api/users/moderation-state`],
                { withCredentials: true }
            );
            const data = res?.data || {};
        ...
        async (u) => {
            ...
            try {
                await tryPost(
                    ...
                    { target_id: targetId, target_type: targetType, action: 'unblock' },
                    { withCredentials: true }
                );
            } catch {
                // ignore
        ...
        async (u) => {
            ...
            try {
                await tryPost(
                    ...
                    { target_id: targetId, target_type: targetType, action: 'unhide' },
                    { withCredentials: true }
                );
            } catch {
                // ignore
    ...
    useEffect(() => {
        ...
        const q = (peopleSearchApplied || '').trim();
        const viewerKey = viewerId || viewerHandle || '';
        const accountKey = activeAccountId || 'personal';
        const oosKey = peopleOutOfState ? `oos|${peopleOosCountry}|${peopleOosState}` : 'local';
        const key = `people|${q}|${peopleCity}|${peopleCounty}|${oosKey}|${viewerKey}|${accountKey}`;

        if (lastAutoFetchKeyRef.current.people !== key) {
            lastAutoFetchKeyRef.current.people = key;
            fetchPeople({ reset: true });
        }
    }, [tab, TAB.PEOPLE, peopleSearchApplied, peopleCity, peopleCounty, peopleOutOfState, peopleOosCountry, peopleOosState, viewerId, viewerHandle, fetchPeople, activeAccountId]);

    // Fetch user location counts for Find People filter badges
    useEffect(() => {
        if (tab !== TAB.PEOPLE) return;
        let cancelled = false;
        (async () => {
            try {
                const params = new URLSearchParams();
                if (peopleSearchApplied) params.set('search', peopleSearchApplied);
                if (peopleCounty) params.set('county', peopleCounty);
                if (peopleCity) params.set('city', peopleCity);
                const res = await secureFetch(`${api}/users/location-counts?${params.toString()}`, { credentials: 'include' });
                if (!res.ok || cancelled) return;
                const data = await res.json();
                if (!cancelled) setPeopleLocationCounts(data || { counties: {}, cities: {} });
            } catch {
                // ignore
            }
        })();
        return () => { cancelled = true; };
    }, [tab, TAB.PEOPLE, peopleSearchApplied, peopleCounty, peopleCity]);

    // Fetch out-of-state location counts (country + state) from server
    useEffect(() => {
        if (tab !== TAB.PEOPLE || !peopleOutOfState) {
            return;
        }
        let cancelled = false;
        (async () => {
            try {
                const params = new URLSearchParams();
                if (peopleSearchApplied) params.set('search', peopleSearchApplied);
                if (peopleOosCountry) params.set('country', peopleOosCountry);
                const res = await secureFetch(`${api}/users/oos-location-counts?${params.toString()}`, { credentials: 'include' });
                if (!res.ok || cancelled) return;
                const data = await res.json();
                if (!cancelled) setPeopleOosLocationCounts(data || { countries: {}, states: {} });
            } catch {
                // ignore
            }
        })();
        return () => { cancelled = true; };
    }, [tab, TAB.PEOPLE, peopleOutOfState, peopleSearchApplied, peopleOosCountry]);

    // Fetch mutual followers for visible people cards
    useEffect(() => {
        if (tab !== TAB.PEOPLE || !isLoggedIn) return;
        const ids = (peopleFiltered || [])
            .map((u) => Number(u?._owner_user_id || u?.id || 0))
            .filter((id) => id > 0 && !mutualFollowersMap[id]);
        if (ids.length === 0) return;
        let cancelled = false;
        (async () => {
            try {
                const res = await secureFetch(`${api}/users/mutual-followers?ids=${ids.join(',')}`, { credentials: 'include' });
                if (!res.ok || cancelled) return;
                const data = await res.json();
                if (!cancelled && data && typeof data === 'object') {
                    setMutualFollowersMap((prev) => ({ ...prev, ...data }));
                }
            } catch {
                // ignore
            }
        ...
        return () => { cancelled = true; };
    }, [tab, TAB.PEOPLE, isLoggedIn, peopleFiltered]);

    useEffect(() => {
        // SAFETY / MODERATION: only fetch when active.
        // Blocks/hides are unified across all accounts — same data regardless of active account.
        if (tab !== TAB.SAFETY) return;

        const viewerKey = viewerId || viewerHandle || '';
        const key = `safety|${viewerKey}`;

        if (lastAutoFetchKeyRef.current.safety !== key) {
            lastAutoFetchKeyRef.current.safety = key;
            fetchModeration();
        }
    }, [tab, TAB.SAFETY, viewerId, viewerHandle, fetchModeration]);
    ...
    // Fetch follow requests when Requests tab is active or on mount for badge count
    useEffect(() => {
        if (!viewerIsPrivate || !isLoggedIn) return;
        fetchFollowRequests();
    }, [viewerIsPrivate, isLoggedIn, fetchFollowRequests]);

    useEffect(() => {
        if (tab !== TAB.REQUESTS) return;
        fetchFollowRequests();
        setFollowRequestsSeen(true);
    }, [tab, TAB.REQUESTS, fetchFollowRequests]);
...
// If you block someone from Following/Followers, remove them from those lists immediately,
    ...
    useEffect(() => {
        const filterFeedByTarget = (userId, targetType) => {
            const matchesPost = (p) => {
                if (!p) return false;
                const tt = String(targetType || '').toLowerCase();
                if (tt === 'business') {
                    const bId = Number(p?.business_id ?? p?.businessId ?? p?.businessPageId ?? p?.business_page_id ?? 0);
                    if (bId === userId) return true;
                    const ownerId = Number(p?.businessOwnerId ?? p?.business_owner_id ?? p?.owner_id ?? 0);
                    if (ownerId === userId) return true;
                } else if (tt === 'artist') {
                    const aId = Number(p?.artist_id ?? p?.artistId ?? 0);
                    if (aId === userId) return true;
                } else {
                    const authorId = Number(p?.user_id ?? p?.author_id ?? p?.created_by_user_id ?? 0);
            ...
            setFeedPosts((prev) => prev.filter((p) => !matchesPost(p)));
            setFeedLikes((prev) => prev.filter((p) => !matchesPost(p)));
            setFeedReposts((prev) => prev.filter((p) => !matchesPost(p)));
            setFeedComments((prev) => prev.filter((c) => !matchesPost(c?.post || c)));
        };
        ...
        const onBlockedChanged = (e) => {
            ...
            if (blocked) {
                ...
                // Immediately remove blocked user's posts from all feed arrays
                filterFeedByTarget(userId, targetType);
            }
            fetchSocial();
        ...
        const onHiddenChanged = (e) => {
            ...
            if (hidden) {
                // Immediately remove hidden user's posts from all feed arrays
                filterFeedByTarget(userId, targetType);
            }
...
// This keeps the Social page "fresh" on every load and prevents stale searches from persisting.
    ...
    const saveSocialState = useCallback(
        ...
        [viewer, tab, feedTab, feedRenderCount, feedPostsOffset, peopleSearch, peopleSearchApplied, peopleCounty, peopleCity]
    );
    ...
    const onPeopleSearch = () => {
        setPeopleSearchApplied(peopleSearch.trim());
        fetchPeople({ search: peopleSearch.trim() });
    };
    ...
    // Auto-apply following/followers county & city filters when they change
    useEffect(() => {
        setFollowingCountyApplied(String(followingCounty || ''));
        setFollowingCityApplied(String(followingCity || ''));
    }, [followingCounty, followingCity]);

    useEffect(() => {
        setFollowersCountyApplied(String(followersCounty || ''));
        setFollowersCityApplied(String(followersCity || ''));
    }, [followersCounty, followersCity]);

    const onPeopleClear = () => {
        setPeopleSearch('');
        setPeopleSearchApplied('');
        setPeopleCounty('');
        setPeopleCity('');
        setPeopleOosCountry('');
        setPeopleOosState('');
        setBizOffset(0); setBizHasMore(true); bizHasMoreRef.current = true;
        setArtOffset(0); setArtHasMore(true); artHasMoreRef.current = true;
    ...
    const onRefresh = async () => {
        ...
        try {
            ...
            else if (tab === TAB.REQUESTS) await fetchFollowRequests();
    ...
    const toolbarPaperSx = {
        ...
        p: { xs: 1, sm: 1.25 },
        bgcolor: { xs: theme.palette.background.paper, sm: alpha(theme.palette.background.paper, 0.96) },
        ...
        overflow: 'visible',
        // Mobile: this is regular scroll content now — the profile sidebar
        // above handles the floating chrome fade. No opacity/position
        // overrides needed.
    };

    const pageShellSx = {
        bgcolor: 'background.default',
        // ── Edge-to-edge content on mobile/tablet ──
        // Shell runs the full viewport; the global header, in-page toolbar/
        // sidebar/feed-tabs refs, and bottom nav float OVER it and fade via
        // `--ll-nav-offset`. Shell top/bottom never change, so scroll
        // position is stable through every fade.
        // Desktop still uses the measured `chromeTop`.
        position: 'fixed',
        top: isMobile ? 0 : `${chromeTop}px`,
        left: 0,
        right: 0,
        bottom: 0,
        overflow: 'hidden',
    ...
    const contentPaperSx = {
        ...
        overflow: 'hidden',
        borderRadius: { xs: 0, sm: 3 },
        bgcolor: { xs: theme.palette.background.paper, sm: theme.palette.background.paper },
        borderColor: { xs: 'transparent', sm: alpha(theme.palette.text.primary, 0.08) },
    ...
    const sidebarNavItem = (isActive) => (t) => ({
        display: 'flex',
        alignItems: 'center',
        ...
        bgcolor: isActive
            ? { xs: 'transparent', md: alpha(t.palette.primary.main, 0.08) }
            : 'transparent',
        border: { xs: 'none', md: '1px solid' },
        borderColor: { xs: 'transparent', md: isActive ? alpha(t.palette.primary.main, 0.2) : 'transparent' },
        boxShadow: 'none',
        flexShrink: { xs: 1, md: 0 },
        transition: `all ${t.custom?.motion?.base || 200}ms ${t.custom?.motion?.ease || 'ease'}`,
        '&:hover': {
            bgcolor: isActive
                ? { xs: 'transparent', md: alpha(t.palette.primary.main, 0.1) }
                : { xs: alpha(t.palette.text.primary, 0.03), md: alpha(t.palette.text.primary, 0.04) },
        },
        userSelect: 'none',
        whiteSpace: 'nowrap',
    });
    ...
    const isNonFeedLoading = (tab === TAB.PEOPLE && loadingPeople) || (tab === TAB.SAFETY && loadingSafety) || (tab === TAB.REQUESTS && loadingFollowRequests);
    ...
    const renderUserGrid = (list, topPad = 0, mode = 'people') => {
        ...
        const rawRenderCount = mode === 'people' ? peopleRenderCount : mode === 'following' ? followingRenderCount : mode === 'followers' ? followersRenderCount : full.length;
        // When a client-side filter is active, always show all filtered results
        const renderCount = (mode === 'people' && peopleOutOfState)
            ? Math.max(rawRenderCount, full.length)
            : rawRenderCount;
        ...
        const handleScroll = (e) => {
            ...
            if (mode === 'people') {
                const loaded = full.length;
                if (peopleRenderCount < loaded) {
                    // Show more already-loaded rows
                    setPeopleRenderCount((c) => Math.min(c + PAGE_SIZE, loaded));
                } else if (!loadingPeopleRef.current && !loadingMorePeopleRef.current && peopleHasMoreRef.current) {
                    // All loaded rows are visible — fetch more from server
        ...
        const getAction = (u) => {
            ...
            const isRequested = locallyRequested.has(tid);

            if (mode === 'followers') {
                if (isFollowingNow) return { label: 'Following', variant: 'outlined', disabled: false, onClick: () => handleFollowToggle(u, 'unfollow') };
                if (isRequested) return { label: 'Requested', variant: 'outlined', disabled: true, onClick: null };
                return { label: 'Follow back', variant: 'contained', disabled: false, onClick: () => handleFollowToggle(u, 'follow') };
            }
            if (mode === 'following') {
                return { label: 'Following', variant: 'outlined', disabled: false, onClick: () => handleFollowToggle(u, 'unfollow') };
            }
            // people
            if (isFollowingNow) return { label: 'Following', variant: 'outlined', disabled: false, onClick: () => handleFollowToggle(u, 'unfollow') };
            if (isRequested) return { label: 'Requested', variant: 'outlined', disabled: true, onClick: null };
        ...
        return (
            <Box data-social-people-scroll onScroll={(e) => { handleScroll(e); }} sx={{
                height: '100%',
                overflowY: 'auto',
                overflowX: 'hidden',
                pr: { xs: 0, md: 1 },
                ...
                // (global header + profile sidebar at top, bottom nav at
                // bottom on phone).
                '@media (max-width: 1199px)': {
                    paddingTop: 'calc(var(--ll-nav-height, 52px) + var(--ll-subheader-height, 52px))',
                },
                ...
                <Box
                    sx={{
                        ...
                        columnGap: { xs: 0, md: GAP_U },
                        rowGap: { xs: 0, md: GAP_U },
                        pb: 2,
                        px: { xs: 0, md: 0.5 },
                        py: { xs: 0, md: 0.5 },
                    }}
                >
                    {(sliced || []).map((u) => {
                        ...
                        const countyRaw = String(cached?.county || u?.home_county || u?.county || '').trim();
                        const state = String(cached?.state || u?.home_state || u?.state || '').trim();
                        const st = state.toLowerCase();
                        const isAlabama = !st || st === 'al' || st === 'alabama';
                        // For Alabama users, append "County" to the county name on the card
                        const county = countyRaw && isAlabama
                            ? `${normalizeCountyName(countyRaw)} County`
                            : countyRaw;
                        ...
                        if (locText && !isAlabama) {
                            locText = `${locText}, ${state}`;
                        }
                        return (
                            <Box key={u?.id || u?.public_id || u?.handle} sx={(t) => ({ width: '100%', borderBottom: { xs: `1px solid ${alpha(t.palette.divider, 0.12)}`, md: 'none' }, '&:last-child': { borderBottom: 'none' } })}>
                                <UserCard
                                    user={{ ...u, ...(cached || {}) }}
                                    ...
                                    mutualFollowers={mutualFollowersMap[Number(u?._owner_user_id || u?.id)] || null}
                                />
                            </Box>
                        );
                    })}
                </Box>
                {mode === 'people' && loadingMorePeople && (
...
// Reset paging whenever switching feed tab or changing filters
    ...
    // ── Category & account-type counts (computed from loaded posts) ──
    const feedCategoryCounts = useMemo(() => {
        const list = Array.isArray(activeRawList) ? activeRawList : [];
        const accountType = String(activeFilter?.accountType || '').trim().toLowerCase();
        // Apply account type filter first so counts reflect the filtered set
        const filtered = accountType ? list.filter((p) => getPostAccountType(p) === accountType) : list;
        const counts = {};
        filtered.forEach((p) => {
        ...
        return counts;
    }, [activeRawList, activeFilter?.accountType]);

    const feedAccountTypeCounts = useMemo(() => {
        const list = Array.isArray(activeRawList) ? activeRawList : [];
        const counts = { personal: 0, business: 0, artist: 0 };
        list.forEach((p) => {
            const t = getPostAccountType(p);
    ...
    const { pullRef: socialPullRef, pullIndicator } = usePullToRefresh({
        onRefresh: () => fetchFeedActivity({ mode: 'reset' }),
        disabled: !isMobile,
    });
...
// Restore Social page state when returning from Post Page
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (raw) {
                ...
                if (typeof snap?.peopleSearchApplied === 'string') setPeopleSearchApplied(snap.peopleSearchApplied);
    ...
    const countText = useMemo(() => {
        ...
        if (tab === TAB.FEED) {
            ...
            const feedWord = feedTab === 2 ? word(total, 'liked post', 'liked posts') : feedTab === 3 ? word(total, 'repost', 'reposts') : feedTab === 1 ? word(total, 'comment group', 'comment groups') : word(total, 'post', 'posts');
            return `Displaying ${shown} of ${total} ${feedWord}`;
        }

        if (tab === TAB.PEOPLE) {
            if (loadingPeople) return 'Loading…';
            const filtered = Array.isArray(peopleFiltered) ? peopleFiltered : [];
            const shown = Math.min(peopleRenderCount, filtered.length);
            // Server-side filtering handles out-of-state, so peopleTotal is always accurate.
            const total = Number.isFinite(peopleTotal) && peopleTotal > filtered.length ? peopleTotal : filtered.length;
        ...
        if (tab === TAB.REQUESTS) {
            const total = followRequests.length;
            return `Displaying ${total} of ${total} ${total === 1 ? 'request' : 'requests'}`;
        }
                    ...
                    >
                        ...
                        <Paper
                            ...
                            sx={{
                                ...
                                boxShadow: { xs: 'none', sm: `0 2px 12px ${alpha(theme.palette.text.primary, 0.05)}` },
                                border: { xs: 'none', sm: undefined },
                                p: { xs: '4px 2px 0px', md: 1 },
                                // Mobile: fixed in viewport directly below the global
                                // header. Doesn't take layout space — the scroll
                                // containers below reserve space via padding-top. Fades
                                // via `--ll-nav-offset` in sync with the rest of the
                                // chrome.
                                ...(isMobile ? {
                                    position: 'fixed',
                        ...
                        >
                            ...
                            {isLoggedIn && (
                                ...
                                >
                                    ...
                                    <Box sx={{ minWidth: 0, overflowX: 'hidden', overflowY: 'visible' }}>
                                        <Typography
                                            noWrap
                                            sx={{
                                                fontWeight: 750,
                                        ...
                                        {isBusinessAccount ? (
                                            <Typography
                                                variant="caption"
                                                noWrap
                                                sx={{ color: 'text.secondary', fontWeight: 500, lineHeight: 1.2, display: 'block' }}
                                            >
                            ...
                            {isLoggedIn && (
                                ...
                                >
                                    <DynamicFeedIcon sx={{ fontSize: { xs: 18, md: 20 } }} />
                                    <Box component="span" sx={{ display: { xs: 'inline', md: 'inline' }, fontSize: { xs: '0.6rem', md: '0.875rem' } }}>Feed</Box>
                                </Box>
                            )}
                            ...
                            >
                                <PersonSearchIcon sx={{ fontSize: { xs: 18, md: 20 } }} />
                                <Box component="span" sx={{ display: { xs: 'inline', md: 'inline' }, fontSize: { xs: '0.6rem', md: '0.875rem' } }}>People</Box>
                            </Box>
                            ...
                            >
                                <PeopleAltOutlinedIcon sx={{ fontSize: { xs: 18, md: 20 } }} />
                                <Box component="span" sx={{ display: { xs: 'inline', md: 'inline' }, fontSize: { xs: '0.6rem', md: '0.875rem' } }}>
                                    <Box component="span" sx={{ display: { xs: 'none', md: 'inline' } }}>Following{isLoggedIn ? ` (${safeNumber(counts.following)})` : ''}</Box>
                            ...
                            >
                                <PersonAddAltOutlinedIcon sx={{ fontSize: { xs: 18, md: 20 } }} />
                                <Box component="span" sx={{ display: { xs: 'inline', md: 'inline' }, fontSize: { xs: '0.6rem', md: '0.875rem' } }}>
                                    <Box component="span" sx={{ display: { xs: 'none', md: 'inline' } }}>Followers{isLoggedIn ? ` (${safeNumber(counts.followers)})` : ''}</Box>
                                    <Box component="span" sx={{ display: { xs: 'inline', md: 'none' } }}>Followers</Box>
                                </Box>
                            </Box>

                            {isLoggedIn && viewerIsPrivate && !isBusinessAccount && !isArtistAccount && (
                                <Box
                                    onClick={() => { setTab(TAB.REQUESTS); setFollowRequestsSeen(true); }}
                                    sx={sidebarNavItem(tab === TAB.REQUESTS)}
                                >
                                    <HowToRegRoundedIcon sx={{ fontSize: { xs: 18, md: 20 } }} />
                                    <Box component="span" sx={{ display: { xs: 'inline', md: 'inline' }, fontSize: { xs: '0.6rem', md: '0.875rem' }, position: 'relative' }}>
                                        <Box component="span" sx={{ display: { xs: 'none', md: 'inline' } }}>
                                            Requests
                                            {followRequestsCount > 0 && !followRequestsSeen && (
                                                <Box
                                                    component="span"
                                                    sx={(t) => ({
                                                        ml: 0.75,
                                        ...
                                        <Box component="span" sx={{ display: { xs: 'inline', md: 'none' } }}>Requests</Box>
                                    </Box>
                                </Box>
                            )}
                            ...
                            {isLoggedIn && (
                                ...
                                >
                                    <ShieldIcon sx={{ fontSize: { xs: 18, md: 20 } }} />
                                    <Box component="span" sx={{ display: { xs: 'inline', md: 'inline' }, fontSize: { xs: '0.6rem', md: '0.875rem' } }}>Safety</Box>
                                </Box>
                            )}

                            {/* Divider + Refresh */}
                            <Divider sx={{ my: 0.5, display: { xs: 'none', md: 'block' } }} />
                            <Box sx={{ px: { md: 0.5 }, ml: { xs: 'auto', md: 0 }, display: { xs: 'none', md: 'block' } }}>
                                {/* Desktop: full button */}
                                <Button
                                    size="small"
                                    variant="outlined"
                            ...
                            >
                                    ...
                                    <React.Fragment key={link.to}>
                                        ...
                                        {i < 2 && (
                                            <Typography
                                                ...
                                                sx={(t) => ({
                                                    fontSize: '0.6rem',
                                                    color: alpha(t.palette.text.disabled, 0.5),
                                                    lineHeight: 1,
                                                    userSelect: 'none',
                                                })}
                                            >
                                ...
                                <Typography
                                    sx={(t) => ({
                                        ...
                                        fontSize: '0.62rem',
                                        fontWeight: 500,
                                        color: alpha(t.palette.text.disabled, 0.55),
                                        mt: 0.25,
                                        lineHeight: 1,
                                    })}
                                >
                                    © {new Date().getFullYear()} Local Lantern
                                </Typography>
                            </Box>
                        </Paper>

                        {/* ═══════ MAIN CONTENT AREA ═══════ */}
                        <Box sx={{ flex: 1, minWidth: 0, minHeight: 0, display: 'flex', flexDirection: 'column', gap: { xs: 0, sm: 1.25 }, height: '100%' }}>

                            {/* ── Inline Post Detail View (replaces feed when a post is selected) ── */}

                            {/* ── Mobile: fullscreen slide-in from right ── */}
                            {isMobile && (
                                <Slide direction="left" in={!!previewPost} mountOnEnter unmountOnExit>
                                    <Box
                                        sx={{
                                            position: 'fixed',
                                            top: 0,
                                    ...
                                    >
                                        ...
                                        <Box
                                            sx={(t) => ({
                                                ...
                                                // The outer container is position:fixed; top:0, so without
                                                // this the back arrow sits behind the system clock/sensors
                                                // on notched devices.
                                                ...topInsetSx({ basePadding: 8, baseMinHeight: 48 }),
                                                borderBottom: '1px solid',
                                        ...
                                        >
                                            ...
                                            <Typography sx={{ fontWeight: 800, fontSize: '1rem' }}>Post</Typography>
                                        </Box>

                                        {/* Post detail content */}
                                        <Box ref={previewScrollBoxRef} sx={{ flex: 1, minHeight: 0, overflowY: 'auto', overflowX: 'hidden', WebkitOverflowScrolling: 'touch' }}>
                                            {previewPost && detectPostKind(previewPost) === 'artist' && (
                                                <MusicPostDetailPanel
                                                    post={previewPost}
                                                    user={viewer}
                                                    onBack={() => { setPreviewPost(null); setPreviewCommentId(null); }}
                                                    onLocationClick={() => {}}
                                                    scrollToCommentId={previewCommentId || undefined}
                                                    highlightCommentId={previewCommentId || undefined}
                                                />
                                            )}
                                            {previewPost && detectPostKind(previewPost) === 'business' && (
                                            ...
                                            {previewPost && detectPostKind(previewPost) === 'user' && (
                                                <PostPage
                                                    embedded
                                                    post={previewPost}
                                                    user={viewer}
                            ...
                            {/* ── Desktop: inline post detail (original behaviour) ── */}
                            {!isMobile && previewPost && (
                                <Fade in timeout={250}>
                                    <Paper
                                        variant="outlined"
                                        sx={{
                                    ...
                                    >
                                        ...
                                        {/* Post detail content */}
                                        <Box ref={!isMobile ? previewScrollBoxRef : undefined} sx={{ flex: 1, minHeight: 0, overflowY: 'auto', overflowX: 'hidden' }}>
                            ...
                            {/* ── Feed / People / Following / Followers / Safety (hidden when post detail is open on desktop) ── */}
                            <Box sx={{ display: (!isMobile && previewPost) ? 'none' : 'contents' }}>

                                {/* Contextual toolbar area (search/filter rows) — only for non-Feed tabs */}
                                {tab === TAB.PEOPLE ? (
                                    <Paper variant="outlined" ref={toolbarHeaderRef} sx={toolbarPaperSx}>
                                        {!isLoggedIn && (
                                            <Paper
                                                variant="outlined"
                                                sx={(t) => ({
                                        ...
                                        {/* ── Search input + Filters button + Clear button (row) ── */}
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 0.75, md: 1.25 }, mb: 1 }}>
                                            <Box sx={{ flex: 1, minWidth: 0 }}>
                                                <SearchInput
                                                    placeholder="Search by name or @user"
                                                    value={peopleSearch}
                                                    onChange={(e) => setPeopleSearch(e.target.value)}
                                                    onSearch={onPeopleSearch}
                                                    onClear={() => {
                                                        setPeopleSearch('');
                                                        setPeopleSearchApplied('');
                                                        fetchPeople({ search: '' });
                                            ...
                                            {/* Mobile: icon-only filter button → opens drawer */}
                                            {isMobile ? (
                                                <Tooltip title="Search & Filter" arrow>
                                                    <IconButton
                                                        onClick={() => setMobilePeopleFilterOpen(true)}
                                                        size="small"
                                                        sx={(t) => ({
                                                            width: 38,
                                                            height: 38,
                                                            borderRadius: 999,
                                                            border: '1px solid',
                                                            borderColor: (peopleCounty || peopleCity || peopleOutOfState) ? alpha(t.palette.primary.main, 0.35) : alpha(t.palette.text.primary, 0.12),
                                                            backgroundColor: (peopleCounty || peopleCity || peopleOutOfState) ? alpha(t.palette.primary.main, 0.08) : alpha(t.palette.text.primary, 0.03),
                                                            color: (peopleCounty || peopleCity || peopleOutOfState) ? t.palette.primary.main : t.palette.text.secondary,
                                                            flexShrink: 0,
                                                        })}
                                                        aria-label="Filters"
                                                    >
                                                ...
                                                /* Desktop: existing Filters + Clear buttons */
                                                <>
                                                    <Tooltip title={showPeopleFilters ? 'Hide filters' : 'Show filters'} arrow>
                                                        <Button
                                                            ...
                                                            sx={(t) => ({
                                                                ...
                                                                borderRadius: 999,
                                                                border: '1px solid',
                                                                borderColor: showPeopleFilters ? alpha(t.palette.primary.main, 0.35) : alpha(t.palette.text.primary, 0.12),
                                                                backgroundColor: showPeopleFilters ? alpha(t.palette.primary.main, 0.08) : alpha(t.palette.text.primary, 0.03),
                                                                color: showPeopleFilters ? t.palette.primary.main : t.palette.text.secondary,
                                                                textTransform: 'none',
                                                                fontWeight: 800,
                                                                fontSize: 13,
                                                                px: 1.25,
                                                                ...
                                                                flexShrink: 0,
                                                                '&:hover': {
                                                                    backgroundColor: showPeopleFilters ? alpha(t.palette.primary.main, 0.12) : alpha(t.palette.text.primary, 0.06),
                                                                    borderColor: showPeopleFilters ? alpha(t.palette.primary.main, 0.45) : alpha(t.palette.text.primary, 0.18),
                                                                },
                                                            })}
                                                        >
                                                            Filters
                                                    ...
                                                    <Tooltip title="Clear filters" arrow>
                                                        <span>
                                                            <IconButton
                                                                ...
                                                                disabled={!peopleSearch && !peopleSearchApplied && !peopleCounty && !peopleCity && !peopleOutOfState && !peopleOosCountry && !peopleOosState}
                                                                sx={(t) => ({
                                                                    width: 38,
                                                                    height: 38,
                                                                    borderRadius: 999,
                                        ...
                                        <Collapse in={showPeopleFilters && !isMobile} timeout={200} unmountOnExit sx={{ overflow: 'visible' }}>
                                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.25, mb: 1 }}>
                                                        ...
                                                        control={
                                                            <Checkbox
                                                                ...
                                                                onChange={(e) => {
                                                                    ...
                                                                    setPeopleOutOfState(on);
                                                                    if (on) {
                                                                        // Clear Alabama county/city filters when switching to OOS
                                                                        setPeopleCounty('');
                                                                        setPeopleCity('');
                                                                    } else {
                                                                        // Clear OOS country/state filters when switching back
                                                                        setPeopleOosCountry('');
                                                                        setPeopleOosState('');
                                                                    }
                                                                }}
                                                ...
                                                {/* Conditional filters: County/City when local, Country/State when OOS */}
                                                {peopleOutOfState ? (
                                                    <Box sx={{ display: 'grid', gap: 1.25, gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' } }}>
                                                        {/* Country dropdown with counts */}
                                                        <FormControl size="small" fullWidth sx={CREAM_INPUT_SX}>
                                                            <InputLabel id="people-oos-country" shrink>Country</InputLabel>
                                                            <Select
                                                                labelId="people-oos-country"
                                                                label="Country"
                                                                value={peopleOosCountry}
                                                                onChange={(e) => {
                                                                    const val = e.target.value || '';
                                                                    setPeopleOosCountry(val);
                                                                    setPeopleOosState('');
                                                                    // If they pick US + Alabama → switch back to local mode
                                                                }}
                                                                displayEmpty
                                                            >
                                                                <MenuItem value="">
                                                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                                                                        <Typography sx={{ fontSize: 13, fontWeight: 700 }}>All Countries</Typography>
                                                                        <Typography sx={{ fontSize: 12, fontWeight: 600, color: 'text.secondary', ml: 1 }}>
                                                                            {Object.values(peopleOosLocationCounts?.countries || {}).reduce((s, v) => s + v, 0)}
                                                                        </Typography>
                                                                    </Box>
                                                                </MenuItem>
                                                                {Object.entries(peopleOosLocationCounts?.countries || {})
                                                                    .sort((a, b) => b[1] - a[1])
                                                                    .map(([code, count]) => (
                                                                        <MenuItem key={code} value={code}>
                                                                            <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                                                                                <Typography sx={{ fontSize: 13, fontWeight: 600 }}>{COUNTRY_NAMES[code] || code}</Typography>
                                                                                <Typography sx={{ fontSize: 12, fontWeight: 600, color: 'text.secondary', ml: 1 }}>{count}</Typography>
                                                                            </Box>
                                                                        </MenuItem>
                                                                    ))}
                                                            </Select>
                                                        ...
                                                        {/* State dropdown — only when country is US */}
                                                        {peopleOosCountry === 'US' ? (
                                                            <FormControl size="small" fullWidth sx={CREAM_INPUT_SX}>
                                                                <InputLabel id="people-oos-state" shrink>State</InputLabel>
                                                                <Select
                                                                    labelId="people-oos-state"
                                                                    label="State"
                                                                    value={peopleOosState}
                                                                    onChange={(e) => {
                                                                        const val = e.target.value || '';
                                                                        if (val === 'AL') {
                                                                            setPeopleOutOfState(false);
                                                                        ...
                                                                        setPeopleOosState(val);
                                                                    }}
                                                                    displayEmpty
                                                                >
                                                                    <MenuItem value="">
                                                                        <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                                                                            <Typography sx={{ fontSize: 13, fontWeight: 700 }}>All States</Typography>
                                                                            <Typography sx={{ fontSize: 12, fontWeight: 600, color: 'text.secondary', ml: 1 }}>
                                                                                {Object.values(peopleOosLocationCounts?.states || {}).reduce((s, v) => s + v, 0)}
                                                                            </Typography>
                                                                        </Box>
                                                                    </MenuItem>
                                                                    {Object.entries(peopleOosLocationCounts?.states || {})
                                                                        .sort((a, b) => b[1] - a[1])
                                                                        .map(([code, count]) => (
                                                                            <MenuItem key={code} value={code}>
                                                                                <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                                                                                    <Typography sx={{ fontSize: 13, fontWeight: 600 }}>{US_STATE_NAMES[code] || code}</Typography>
                                                                                    <Typography sx={{ fontSize: 12, fontWeight: 600, color: 'text.secondary', ml: 1 }}>{count}</Typography>
                                                                                </Box>
                                                                            </MenuItem>
                                                                        ))}
                                                    ...
                                                    <CityCountySelect
                                                        county={peopleCounty || ALL_COUNTIES_LABEL}
                                                        setCounty={(v) => {
                                                            const next = v === ALL_COUNTIES_LABEL ? '' : (v || '');
                                                            setPeopleCity('');
                                                            setPeopleCounty(next);
                                                        }}
                                                        city={peopleCity || ALL_CITIES_LABEL}
                                                        setCity={(v) => {
                                                            const next = v === ALL_CITIES_LABEL ? '' : (v || '');
                                                            setPeopleCity(next);
                                                        }}
                                                        onCityCountyChange={({ city: nextCity, county: nextCounty }) => {
                                                            setPeopleCity(nextCity === ALL_CITIES_LABEL ? '' : nextCity);
                                                            setPeopleCounty(nextCounty === ALL_COUNTIES_LABEL ? '' : nextCounty);
                                                        }}
                                                        allCountyValue={ALL_COUNTIES_LABEL}
                                                        allCityValue={ALL_CITIES_LABEL}
                                                        countyCounts={peopleLocationCounts?.counties || null}
                                                        cityCounts={peopleLocationCounts?.cities || null}
                                ...
                                ) : (tab === TAB.FOLLOWING || tab === TAB.FOLLOWERS) && isLoggedIn ? (
                                    <Paper variant="outlined" ref={toolbarHeaderRef} sx={toolbarPaperSx}>
                                        {/* Search row */}
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 0.75, md: 1 }, mb: 0 }}>
                                            <Box sx={{ flex: 1, minWidth: 0 }}>
                                                <SearchInput
                                                    placeholder={tab === TAB.FOLLOWING ? 'Search following…' : 'Search followers…'}
                                                    ...
                                                    onClear={() => {
                                                        if (tab === TAB.FOLLOWING) { setFollowingSearch(''); setFollowingSearchApplied(''); }
                                                        else { setFollowersSearch(''); setFollowersSearchApplied(''); }
                                                    }}
                                                />
                                            </Box>
                                            {/* Reset filters icon */}
                                            {(() => {
                                                const hasAny = tab === TAB.FOLLOWING
                                                    ? (followingSearch || followingSearchApplied || followingCounty || followingCity || followingCountyApplied || followingCityApplied || followingOutOfState || followingAccountType)
                                                    : (followersSearch || followersSearchApplied || followersCounty || followersCity || followersCountyApplied || followersCityApplied || followersOutOfState || followersAccountType);
                                                return (
                                                    <Tooltip title="Reset all filters" arrow>
                                                        <span>
                                                            <IconButton
                                                                ...
                                                                onClick={() => {
                                                                    if (tab === TAB.FOLLOWING) {
                                                                        setFollowingSearch(''); setFollowingSearchApplied('');
                                                                        setFollowingCounty(''); setFollowingCity('');
                                                                        setFollowingCountyApplied(''); setFollowingCityApplied('');
                                                                        setFollowingOutOfState(false); setFollowingAccountType('');
                                                                    } else {
                                                                        setFollowersSearch(''); setFollowersSearchApplied('');
                                                                        setFollowersCounty(''); setFollowersCity('');
                                                                        setFollowersCountyApplied(''); setFollowersCityApplied('');
                                                                        setFollowersOutOfState(false); setFollowersAccountType('');
                                                                    }
                                                                }}
                                                                sx={(t) => ({
                                                                    width: 38, height: 38, borderRadius: 999,
                                                                    border: '1px solid',
                                                                    borderColor: hasAny ? alpha(t.palette.primary.main, 0.35) : alpha(t.palette.text.primary, 0.12),
                                                                    backgroundColor: hasAny ? alpha(t.palette.primary.main, 0.08) : 'transparent',
                                                                    color: hasAny ? t.palette.primary.main : t.palette.text.disabled,
                                                                    flexShrink: 0,
                                            ...
                                            {isMobile ? (
                                                <Tooltip title="Search & Filter" arrow>
                                                    <IconButton
                                                        onClick={() => setMobileFollowFilterOpen(true)}
                                                        size="small"
                                                        sx={(t) => {
                                                            const hasFilters = tab === TAB.FOLLOWING
                                                                ? (followingCounty || followingCity || followingOutOfState || followingAccountType)
                                                                : (followersCounty || followersCity || followersOutOfState || followersAccountType);
                                                            return {
                                                                width: 38, height: 38, borderRadius: 999,
                                                                border: '1px solid',
                                                                borderColor: hasFilters ? alpha(t.palette.primary.main, 0.35) : alpha(t.palette.text.primary, 0.12),
                                                                backgroundColor: hasFilters ? alpha(t.palette.primary.main, 0.08) : alpha(t.palette.text.primary, 0.03),
                                                                color: hasFilters ? t.palette.primary.main : t.palette.text.secondary,
                                                                flexShrink: 0,
                                                            };
                                                        }}
                                                        aria-label="Filters"
                                        ...
                                        {/* Desktop-only inline filters — county, city, out of state */}
                                        {!isMobile && (
                                            <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mt: 1.5 }}>
                                                <Autocomplete
                                                    size="small"
                                                    sx={{ minWidth: 160, maxWidth: 260, flex: 1 }}
                                                    options={[ALL_COUNTIES_LABEL, ...(Array.isArray(peopleCounties) ? peopleCounties : [])]}
                                                    value={tab === TAB.FOLLOWING ? (followingCounty || ALL_COUNTIES_LABEL) : (followersCounty || ALL_COUNTIES_LABEL)}
                                                    onChange={(_, val) => {
                                                        const str = String(val || '').trim();
                                                        if (tab === TAB.FOLLOWING) {
                                                            setFollowingCity('');
                                                            if (!str || str === ALL_COUNTIES_LABEL) setFollowingCounty(''); else setFollowingCounty(str);
                                                        } else {
                                                            setFollowersCity('');
                                                            if (!str || str === ALL_COUNTIES_LABEL) setFollowersCounty(''); else setFollowersCounty(str);
                                                        }
                                                    }}
                                                    renderInput={(params) => <TextField {...params} label="County" />}
                                                    clearOnEscape autoHighlight filterSelectedOptions
                                                />
                                                <Autocomplete
                                                    size="small"
                                                    sx={{ minWidth: 160, maxWidth: 260, flex: 1 }}
                                                    options={[ALL_CITIES_LABEL, ...(tab === TAB.FOLLOWING ? (Array.isArray(followingCities) ? followingCities : []) : (Array.isArray(followersCities) ? followersCities : []))]}
                                                    value={tab === TAB.FOLLOWING ? (followingCity || ALL_CITIES_LABEL) : (followersCity || ALL_CITIES_LABEL)}
                                                    onChange={(_, val) => {
                                                        const str = String(val || '').trim();
                                                        if (tab === TAB.FOLLOWING) { if (!str || str === ALL_CITIES_LABEL) setFollowingCity(''); else setFollowingCity(str); }
                                                        else { if (!str || str === ALL_CITIES_LABEL) setFollowersCity(''); else setFollowersCity(str); }
                                                    }}
                                                    renderInput={(params) => {
                                                ...
                                                <FormControlLabel
                                                    control={
                                                        <Checkbox
                                                            ...
                                                            onChange={(e) => { if (tab === TAB.FOLLOWING) setFollowingOutOfState(e.target.checked); else setFollowersOutOfState(e.target.checked); }}
                                                            size="small"
                                                            icon={<FlightTakeoffIcon sx={{ fontSize: 20, color: 'text.disabled' }} />}
                                                            checkedIcon={<FlightTakeoffIcon sx={{ fontSize: 20 }} />}
                                                            sx={{ p: 0.5 }}
                                                    ...
                                                    label={<Typography variant="caption" sx={{ fontWeight: 700, lineHeight: 1, whiteSpace: 'nowrap' }}>Out of State</Typography>}
                                                    sx={{ mr: 0, ml: 0, flexShrink: 0 }}
                                                />
                                            </Box>
                                        )}

                                        {/* Desktop-only advanced filter row (account type chips) */}
                                        {!isMobile && (
                                            <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 1, mt: 1, pt: 1, borderTop: '1px solid', borderColor: (t) => alpha(t.palette.text.primary, 0.06) }}>
                                                <TuneIcon sx={{ fontSize: 18, color: 'text.secondary', mr: 0.25 }} />
                                                <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary', mr: 0.5 }}>Filters:</Typography>
                                                {ACCOUNT_TYPE_OPTIONS.filter((o) => o.value).map((o) => {
                                                    const currentType = tab === TAB.FOLLOWING ? followingAccountType : followersAccountType;
                                                    return (
                                                        <Chip key={o.value} label={o.label} size="small" variant={currentType === o.value ? 'filled' : 'outlined'} color={currentType === o.value ? 'primary' : 'default'} onClick={() => { if (tab === TAB.FOLLOWING) setFollowingAccountType((prev) => prev === o.value ? '' : o.value); else setFollowersAccountType((prev) => prev === o.value ? '' : o.value); }} sx={{ fontWeight: 700, cursor: 'pointer', fontSize: '0.75rem' }} />
                                                    );
                                                })}
                                            </Box>
                                        )}
                                ...
                                <Paper variant="outlined" sx={tab === TAB.FEED ? { ...contentPaperSx, ...feedContentPaperSx } : { ...contentPaperSx, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                                    <Box sx={{
                                        ...
                                        {isNonFeedLoading && tab !== TAB.FEED ? (
                                            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', py: 10, width: '100%' }}>
                                                <PulsingDots />
                                            </Box>
                                        ) : (
                                            <>
                                                {tab === TAB.FEED ? (
                                                    ...
                                                    >
                                                        {/* Feed tabs row — scrolls as regular content */}
                                                        <Box
                                                            ref={feedTabsRef}
                                                            sx={{
                                                                flexShrink: 0,
                                                                // Mobile: regular scroll content — the profile sidebar
                                                                // above handles the floating chrome fade.
                                                            }}
                                                        >
                                                            {/* Feed tabs row — on mobile, search icon sits next to tabs */}
                                                            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                                                <Tabs value={feedTab} onChange={(_, v) => setFeedTab(v)} variant="fullWidth" sx={{
                                                                    ...tabStyles,
                                                                    flex: 1,
                                                                    minHeight: { xs: 36, sm: 52 },
                                                                    '& .MuiTab-root': {
                                                                        ...tabStyles(theme)['& .MuiTab-root'],
                                                                        minHeight: { xs: 36, sm: 52 },
                                                                        minWidth: { xs: 0 },
                                                                        px: { xs: 0.5, sm: 2 },
                                                                        py: { xs: 0.35, sm: 1 },
                                                                    ...
                                                                    <Tab
                                                                        ...
                                                                        {...(!isMobile && { iconPosition: 'start', label: 'Posts' })}
                                                                    />
                                                                    <Tab
                                                                        icon={(
                                                                            <TabIconWrapper size={24}>
                                                                        ...
                                                                        {...(!isMobile && { iconPosition: 'start', label: 'Comments' })}
                                                                    />
                                                                    <Tab
                                                                        icon={(
                                                                        ...
                                                                        {...(!isMobile && { iconPosition: 'start', label: 'Likes' })}
                                                                    />
                                                                    <Tab
                                                                        icon={(
                                                                            <TabIconWrapper size={24}>
                                                                        ...
                                                                        {...(!isMobile && { iconPosition: 'start', label: 'Reposts' })}
                                                                    />
                                                                </Tabs>

                                                                {/* Mobile: search icon next to tabs */}
                                                                {isMobile && (
                                                                    <IconButton
                                                                        ...
                                                                        sx={(t) => ({
                                                                            ...
                                                                            mx: 0.75,
                                                                            flexShrink: 0,
                                                                            color: (activeFilter?.searchTerm || activeFilter?.dateFrom || activeFilter?.dateTo || activeFilter?.accountType || activeFilter?.category || (activeFilter?.sort && activeFilter.sort !== 'any'))
                                                                                ? t.palette.primary.main
                                                                                : alpha(t.palette.text.secondary, 0.7),
                                                                            '&:hover': {
                                                                                bgcolor: alpha(t.palette.text.primary, 0.04),
                                                                            },
                                                                        })}
                                                            ...
                                                            {/* Search + Filters button row — desktop only */}
                                                            <Box
                                                                sx={{
                                                                    display: { xs: 'none', md: 'flex' },
                                                                    alignItems: 'center',
                                                            ...
                                                            >
                                                                <Box sx={{ flex: 1, minWidth: 0 }}>
                                                                    <SearchInput
                                                                        ...
                                                                        onClear={() => updateFeedFilter({ searchTerm: '' })}
                                                                    />
                                                                </Box>
                                                                {/* Desktop: Filters button + Clear button */}
                                                                <Tooltip title={showFeedFilters ? 'Hide filters' : 'Show filters'} arrow>
                                                                    <Button
                                                                        ...
                                                                        sx={(t) => ({
                                                                            ...
                                                                            borderRadius: 999,
                                                                            border: '1px solid',
                                                                            borderColor: showFeedFilters ? alpha(t.palette.primary.main, 0.35) : alpha(t.palette.text.primary, 0.12),
                                                                            backgroundColor: showFeedFilters ? alpha(t.palette.primary.main, 0.08) : alpha(t.palette.text.primary, 0.03),
                                                                            color: showFeedFilters ? t.palette.primary.main : t.palette.text.secondary,
                                                                            textTransform: 'none',
                                                                            fontWeight: 800,
                                                                            fontSize: 13,
                                                                            px: 1.25,
                                                                            ...
                                                                            flexShrink: 0,
                                                                            '&:hover': {
                                                                                backgroundColor: showFeedFilters ? alpha(t.palette.primary.main, 0.12) : alpha(t.palette.text.primary, 0.06),
                                                                                borderColor: showFeedFilters ? alpha(t.palette.primary.main, 0.45) : alpha(t.palette.text.primary, 0.18),
                                                                            },
                                                                        })}
                                                                    >
                                                                        Filters
                                                                ...
                                                                <Tooltip title="Clear filters" arrow>
                                                                    <span>
                                                                        <IconButton
                                                                            ...
                                                                            disabled={!(activeFilter?.dateFrom || activeFilter?.dateTo || activeFilter?.searchTerm || activeFilter?.accountType || activeFilter?.category || (activeFilter?.sort && activeFilter.sort !== 'any'))}
                                                                            sx={(t) => ({
                                                                                width: 38,
                                                                                height: 38,
                                                                                borderRadius: 999,
                                                            ...
                                                            {/* Collapsible filter panel — desktop only (mobile uses full-screen drawer) */}
                                                            <Collapse in={showFeedFilters && !isMobile} timeout={200} unmountOnExit sx={{ overflow: 'visible' }}>
                                                                <Box
                                                                    sx={{
                                                                        px: 1.5,
                                                                        pt: 2.5,
                                                                ...
                                                                >
                                                                    {/* All filters — wraps on mobile, scrolls on desktop */}
                                                                    <Box
                                                                        sx={{
                                                                            display: 'flex',
                                                                            gap: 0.75,
                                                                    ...
                                                                    >
                                                                        {/* Account Type */}
                                                                        <FormControl size="small" sx={{ minWidth: 115, flexShrink: 0, ...CONTROL_SX }}>
                                                                            ...
                                                                            <Select
                                                                                ...
                                                                                renderValue={(val) => {
                                                                                    ...
                                                                                    const ct = val ? (feedAccountTypeCounts[val] || 0) : (Array.isArray(activeRawList) ? activeRawList.length : 0);
                                                                                    return <CategoryRow icon={Icon} label={found.label} count={ct} />;
                                                                                }}
                                                                            >
                                                                                {ACCOUNT_TYPE_OPTIONS.map((o) => {
                                                                                    const icons = { '': PeopleRoundedIcon, personal: PersonRoundedIcon, business: StorefrontOutlinedIcon, artist: MusicNoteOutlinedIcon };
                                                                                    const Icon = icons[o.value] || PeopleRoundedIcon;
                                                                                    const ct = o.value ? (feedAccountTypeCounts[o.value] || 0) : (Array.isArray(activeRawList) ? activeRawList.length : 0);
                                                                                    return (
                                                                                        <MenuItem key={o.value} value={o.value} disabled={o.value && ct === 0}>
                                                                                            <CategoryRow icon={Icon} label={o.label} count={ct} />
                                                                                        </MenuItem>
                                                                                    );
                                                                                })}
                                                                            </Select>
                                                                        ...
                                                                        {(() => {
                                                                            ...
                                                                            // Compute total for "All Categories" row
                                                                            const allCatsTotal = Object.values(feedCategoryCounts).reduce((s, v) => s + v, 0);

                                                                            return (
                                                                                <FormControl size="small" sx={{ minWidth: 160, flexShrink: 0, ...CONTROL_SX }}>
                                                                                    ...
                                                                                    <Select
                                                                                        ...
                                                                                        renderValue={(val) => {
                                                                                            ...
                                                                                            const ct = v ? (feedCategoryCounts[v] || 0) : allCatsTotal;
                                                                                            return <CategoryRow icon={found?.Icon} label={found?.label || 'All Categories'} count={ct} />;
                                                                                        }}
                                                                                    >
                                                                                        {cats.map((c) => {
                                                                                            const ct = c.id ? (feedCategoryCounts[c.id] || 0) : allCatsTotal;
                                                                                            return (
                                                                                                <MenuItem key={c.id || 'all'} value={c.id} disabled={c.id && ct === 0}>
                                                                                                    <CategoryRow icon={c.Icon} label={c.label} count={ct} />
                                                                                                </MenuItem>
                                                                                            );
                                                                                        })}
                                                                                    </Select>
                                                                        ...
                                                                        {/* Sort */}
                                                                        <FormControl size="small" sx={{ minWidth: 95, flexShrink: 0, ...CONTROL_SX }}>
                                                                        ...
                                                                        <TextField
                                                                            ...
                                                                            sx={{ minWidth: 130, maxWidth: 150, flexShrink: 0, ...CONTROL_SX, '& .MuiInputBase-input': { fontSize: 13, fontWeight: 700, letterSpacing: '-0.01em' } }}
                                                                        />
                                                        ...
                                                        <Box
                                                            data-social-feed-scroll
                                                            ref={(node) => { feedScrollRef.current = node; socialPullRef(node); }}
                                                            onScroll={(e) => {
                                                                const el = e.currentTarget;
                                                                if (!el) return;
                                                                const nearBottom = el.scrollTop + el.clientHeight >= el.scrollHeight - 400;
                                                            ...
                                                            sx={(t) => ({
                                                                ...
                                                                pb: 2,
                                                                pt: { xs: 0.75, sm: 1.5 },
                                                                bgcolor: { xs: t.palette.background.paper, md: t.palette.background.paper },
                                                                // Mobile/tablet: reserve space under the floating chrome
                                                                // (global header + profile sidebar at top, bottom nav at
                                                                // bottom on phone).
                                                                '@media (max-width: 1199px)': {
                                                                                    ...
                                                                                    >
                                                                                        ...
                                                                                        >
                                                                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, minWidth: 0 }}>
                                                                                                {(() => {
                                                                                                    const postKind = detectPostKind(post0);
                                                                                                    const pAvatar = String(
                                                                                                        post0?.avatar_url || post0?.profile_picture ||
                                                                                                        post0?.business_avatar_url || post0?.businessLogo ||
                                                                                                        post0?.artistAvatar || post0?.artist_avatar_url || ''
                                                                                                    ).trim();
                                                                                                    const pShowIcon = isDefaultAvatar(pAvatar);
                                                                                                    const pName = String(
                                                                                                        post0?.first_name || post0?.business_name || post0?.businessName ||
                                                                                                        post0?.artist_name || post0?.artistName || post0?.post_author_name || ''
                                                                                                    ).trim();
                                                                                                    const pLast = String(post0?.last_name || '').trim();
                                                                                                    const pFull = `${pName} ${pLast}`.trim();
                                                                                                    const pHandleRaw = String(
                                                                                                        post0?.handle || post0?.business_slug || post0?.businessSlug ||
                                                                                                        post0?.artist_handle || post0?.artistHandle || ''
                                                                                                    ).trim();
                                                                                                    // Visual-artist vs musician fallback icon
                                                                                                    const pProfileType = String(
                                                                                                        post0?.profile_type || post0?.profileType ||
                                                                                                        post0?.artist_profile_type || post0?.artistProfileType || ''
                                                                                                    ).toLowerCase();
                                                                                                    const pIsVisualArtist = postKind === 'artist' && pProfileType === 'artist';
                                                                                                    return (
                                                                                                        <Avatar
                                                                                                            src={!pShowIcon ? pAvatar : undefined}
                                                                                                            alt={pFull || (pHandleRaw ? `@${pHandleRaw}` : 'Post author')}
                                                                                                            sx={(t) => ({
                                                                                                                width: 34,
                                                                                                                height: 34,
                                                                                                                flexShrink: 0,
                                                                                                                ...
                                                                                                                borderColor: 'divider',
                                                                                                                ...(pShowIcon
                                                                                                                        ? (postKind === 'business' || postKind === 'artist')
                                                                                                                            ? { bgcolor: alpha(t.palette.primary.main, 0.08), color: t.palette.primary.main }
                                                                                                                            : { bgcolor: alpha(t.palette.text.primary, 0.06), color: t.palette.text.secondary }
                                                                                                                        : null
                                                                                                                ),
                                                                                        ...
                                                                                        <Box sx={{ px: 1.5, py: 1.25, display: 'grid', gap: 1 }}>
                                                                                            {comments.slice(0, 3).map((c) => {
                                                                                                ...
                                                                                                // Visual-artist vs musician fallback icon (profile_type from
                                                                                                // comment stamp, then cached author record).
                                                                                ...
                                                                                {(feedTab === 2 || feedTab === 3) ? (
                                                                                    <Box className="social-feed-grid" sx={{ display: 'flex', flexWrap: 'wrap', width: '100%', overflowX: 'hidden' }}>
                                                                                        {list.map((p, idx) => {
                                                                                            ...
                                                                                            const actorProfilePath = actorHandleRaw
                                                                                            ...
                                                                                            const attrRow = (actorName || actorHandle) ? (
                                                                                                <Box
                                                                                                    ...
                                                                                                    sx={(t) => ({
                                                                                                        ...
                                                                                                        flexShrink: 0,
                                                                                                        textAlign: 'left',
                                                                                                        borderBottom: `1px solid ${alpha(t.palette.text.primary, 0.05)}`,
                                                                                                        bgcolor: alpha(isRepost ? t.palette.info.main : t.palette.error.main, 0.03),
                                                                                                    })}
                                                                                                >
                                                        ...
                                                        {/* Fixed bottom status bar — hidden on mobile */}
                                                        <Box
                                                            sx={{
                                                                flexShrink: 0,
                                                                borderTop: '1px solid',
                                                ...
                                                {tab === TAB.FOLLOWING ? (
                                                    !isLoggedIn ? (
                                                        <Box sx={{ py: { xs: 8, md: 12 }, px: 3, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center' }}>
                                                            <PeopleAltOutlinedIcon sx={(t) => ({ fontSize: 84, color: alpha(t.palette.primary.main, 0.86), mb: 1.5 })} />
                                                            <Typography sx={{ fontWeight: 800, fontSize: '1.1rem', color: 'text.primary', mb: 0.75 }}>
                                                                Sign up to follow people
                                                            </Typography>
                                                            <Typography sx={{ color: 'text.secondary', fontSize: '0.88rem', maxWidth: 380, mx: 'auto', mb: 2.5 }}>
                                                                Create an account to follow locals in your community and see their posts in your feed.
                                                            </Typography>
                                                            <Button variant="contained" disableElevation onClick={openAuthUI} sx={{ fontWeight: 700, textTransform: 'none', borderRadius: 999, px: 4, py: 1 }}>
                                                                Sign Up
                                                            </Button>
                                                        </Box>
                                                    ) : following.length === 0 && isNetworkError(socialFetchError) ? (
                                                        ...
                                                        renderEmpty('No following found', 'Try a different search or filter.')
                                                    ) : (
                                                ...
                                                {tab === TAB.FOLLOWERS ? (
                                                    !isLoggedIn ? (
                                                        <Box sx={{ py: { xs: 8, md: 12 }, px: 3, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center' }}>
                                                            <PeopleAltOutlinedIcon sx={(t) => ({ fontSize: 84, color: alpha(t.palette.primary.main, 0.86), mb: 1.5 })} />
                                                            <Typography sx={{ fontWeight: 800, fontSize: '1.1rem', color: 'text.primary', mb: 0.75 }}>
                                                                Sign up to see your followers
                                                            </Typography>
                                                            <Typography sx={{ color: 'text.secondary', fontSize: '0.88rem', maxWidth: 380, mx: 'auto', mb: 2.5 }}>
                                                                Create an account to connect with people in your community and build your following.
                                                            </Typography>
                                                            <Button variant="contained" disableElevation onClick={openAuthUI} sx={{ fontWeight: 700, textTransform: 'none', borderRadius: 999, px: 4, py: 1 }}>
                                                                Sign Up
                                                            </Button>
                                                        ...
                                                        renderEmpty('No followers found', 'Try a different search or filter.')
                                                    ) : (
                                                        renderUserGrid(followersFiltered, 0, 'followers')
                                                    )
                                                ) : null}

                                                {tab === TAB.REQUESTS && !isBusinessAccount && !isArtistAccount ? (
                                                    <Box sx={{ py: 2, px: { xs: 1, sm: 2 } }}>
                                                        <Typography sx={{ fontWeight: 950, fontSize: 16, mb: 0.5 }}>Follow Requests</Typography>
                                                        <Typography sx={{ color: 'text.secondary', fontSize: '0.88rem', mb: 2 }}>
                                                            People who want to follow your private account.
                                                        </Typography>
                                                        {followRequests.length === 0 ? (
                                                            renderEmpty('No pending requests', 'When someone requests to follow you, it will appear here.')
                                                        ) : (
                                                            <Box sx={{ display: 'grid', gap: 1.5 }}>
                                                                {followRequests.map((req) => {
                                                                    const name = `${req.first_name || ''} ${req.last_name || ''}`.trim() || 'User';
                                                                    const handle = req.handle ? `@${req.handle}` : '';
                                                                    const avatarUrl = req.profile_picture || '';
                                                                    const profilePath = req.handle ? `/${req.handle}` : req.public_id ? `/${req.public_id}` : `/${req.requester_id}`;
                                                                    return (
                                                                        <Paper
                                                                            key={req.id}
                                                                            variant="outlined"
                                                                        ...
                                                                        >
                                                                            <Avatar
                                                                                ...
                                                                                sx={{ width: 48, height: 48, cursor: 'pointer', border: '2px solid', borderColor: (t) => alpha(t.palette.primary.main, 0.12) }}
                                                                            >
                                                                                <PersonRoundedIcon />
                                                                            </Avatar>
                                                                            <Box sx={{ flex: 1, minWidth: 0 }}>
                                                                                <Typography
                                                                                    sx={{ fontWeight: 900, fontSize: 14, cursor: 'pointer', '&:hover': { textDecoration: 'underline' }, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                                                                                    onClick={() => navigate(profilePath)}
                                                                                >
                                                                                    {name}
                                                                                </Typography>
                                                                            ...
                                                                            <Box sx={{ display: 'flex', gap: 1, flexShrink: 0 }}>
                                                                                <Button
                                                                                    ...
                                                                                    sx={{ borderRadius: 999, textTransform: 'none', fontWeight: 800, fontSize: 12, minWidth: 72, boxShadow: 'none' }}
                                                                                >
                                                                                    Accept
                                                                                </Button>
                                                                                <Button
                                                                                    ...
                                                                                    sx={{ borderRadius: 999, textTransform: 'none', fontWeight: 800, fontSize: 12, minWidth: 72 }}
                                                                                >
                                                                                    Decline
                                                                                </Button>
                                                                            </Box>
                                                    ...
                                                    <Box sx={{ height: '100%', overflow: 'hidden', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                                                        <Box sx={{ flexShrink: 0 }}>
                                                            ...
                                                            <Typography sx={{ color: 'text.secondary', mt: 0.5, mb: 1.5 }}>
                                                                Manage users you've blocked or hidden. These settings apply across all your accounts.
                                                            </Typography>
                                                        </Box>
                                    ...
                                    {/* Count bar — pinned to bottom of content Paper for People/Following/Followers — hidden on mobile */}
                        ...
                        {/* ═══ Mobile full-screen filter drawer (matches BusinessHubPage pattern) ═══ */}
                        {isMobile && (
                            <Drawer
                                anchor="bottom"
                            ...
                            >
                                {/* Header */}
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 2, py: 1.5, ...topInsetSx({ basePadding: 12 }), borderBottom: '1px solid', borderColor: 'divider', flexShrink: 0 }}>
                                ...
                                <Box sx={{ flex: 1, overflow: 'auto', px: 2, pt: 1.5, pb: 2 }}>
                                    <Box
                                        sx={{
                                            ...
                                            backgroundImage: (t) =>
                                                `linear-gradient(180deg, ${alpha(t.palette.primary.main, 0.04)} 0%, ${alpha(t.palette.primary.main, 0.01)} 100%)`,
                                            boxShadow: (t) =>
                                                `0 10px 28px ${alpha(t.palette.text.primary, 0.06)}, inset 0 0 0 1px ${alpha(t.palette.primary.main, 0.08)}`,
                                        }}
                                    >
                                        {/* Account Type */}
                                        <FormControl size="small" fullWidth sx={CONTROL_SX}>
                                            <InputLabel id="social-mobile-accttype" shrink>Type</InputLabel>
                                            <Select
                                                labelId="social-mobile-accttype"
                                                label="Type"
                                                value={activeFilter?.accountType || ''}
                                                onChange={(e) => updateFeedFilter({ accountType: String(e.target.value || ''), category: '' })}
                                                displayEmpty
                                                renderValue={(val) => {
                                                    const found = ACCOUNT_TYPE_OPTIONS.find((o) => o.value === val) || ACCOUNT_TYPE_OPTIONS[0];
                                                    const icons = { '': PeopleRoundedIcon, personal: PersonRoundedIcon, business: StorefrontOutlinedIcon, artist: MusicNoteOutlinedIcon };
                                        ...
                                        {(() => {
                                            ...
                                            return (
                                                <FormControl size="small" fullWidth sx={CONTROL_SX}>
                                                    <InputLabel id="social-mobile-category" shrink>Category</InputLabel>
                                                    <Select
                                                        labelId="social-mobile-category"
                                                        label="Category"
                                                        value={cats.some((c) => c.id === (activeFilter?.category || '')) ? (activeFilter?.category || '') : ''}
                                                        onChange={(e) => updateFeedFilter({ category: String(e.target.value || '') })}
                                                        displayEmpty
                                                        renderValue={(val) => {
                                                            const v = String(val || '').trim().toLowerCase();
                                                            const found = cats.find((c) => c.id === v) || cats[0];
                                                            const CatIcon = found?.Icon || null;
                                                            return (
                                                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, minWidth: 0 }}>
                                                                    {CatIcon ? <CatIcon sx={{ fontSize: 20, color: 'primary.main', flexShrink: 0 }} /> : <Box sx={{ width: 20, flexShrink: 0 }} />}
                                                                    <Typography sx={{ fontSize: '0.875rem', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                                        {found?.label || 'All Categories'}
                                                                    </Typography>
                                                                </Box>
                                                            );
                                                        }}
                                                    >
                                                        {cats.map((c) => {
                                                            const CatIcon = c.Icon || null;
                                                            return (
                                                                <MenuItem key={c.id || 'all'} value={c.id}>
                                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                                        {CatIcon ? <CatIcon sx={{ fontSize: 20, color: 'primary.main' }} /> : <Box sx={{ width: 20 }} />}
                                                                        {c.label}
                                                                    </Box>
                                                                </MenuItem>
                                                            );
                                        ...
                                        {/* Sort */}
                                        <FormControl size="small" fullWidth sx={CONTROL_SX}>
                                            <InputLabel id="social-mobile-sort" shrink>Sort by</InputLabel>
                                            <Select
                                                labelId="social-mobile-sort"
                                                label="Sort by"
                                                value={activeFilter?.sort || 'any'}
                                                onChange={(e) => updateFeedFilter({ sort: String(e.target.value || 'any') })}
                                                renderValue={(value) => {
                                                    ...
                                                    return (
                                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, minWidth: 0 }}>
                                                            <SortRoundedIcon sx={{ fontSize: 20, color: 'primary.main', flexShrink: 0 }} />
                                                            <Typography sx={{ fontSize: '0.875rem', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                                {option?.label || 'Any'}
                                                            </Typography>
                                                        </Box>
                                        ...
                                        <Box sx={{ display: 'flex', gap: 1 }}>
                                            <TextField
                                                ...
                                                onChange={(e) => updateFeedFilter({ dateFrom: e.target.value, dateRange: 'all' })}
                                                fullWidth
                                                sx={{ ...CONTROL_SX, '& .MuiInputBase-input': { fontSize: 14, fontWeight: 700, letterSpacing: '-0.01em' } }}
                                            />
                                            <TextField
                                                size="small"
                                                type="date"
                        ...
                        {/* ═══ Mobile People filter drawer ═══ */}
                        {isMobile && (
                            <Drawer
                                anchor="bottom"
                                open={mobilePeopleFilterOpen}
                                onClose={() => setMobilePeopleFilterOpen(false)}
                                PaperProps={{ sx: { height: '100vh', borderRadius: 0, overflow: 'hidden', bottom: 0, display: 'flex', flexDirection: 'column' } }}
                                ModalProps={{ keepMounted: true }}
                                slotProps={{ backdrop: { sx: { bottom: 0 } } }}
                                sx={{ zIndex: (t) => t.zIndex.drawer + 2 }}
                            >
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 2, py: 1.5, ...topInsetSx({ basePadding: 12 }), borderBottom: '1px solid', borderColor: 'divider', flexShrink: 0 }}>
                                    <IconButton onClick={() => setMobilePeopleFilterOpen(false)} size="small" sx={{ width: 36, height: 36 }}><ArrowBackRoundedIcon sx={{ fontSize: 22 }} /></IconButton>
                                    <Typography sx={{ fontWeight: 900, fontSize: 16, flex: 1 }}>Search & Filter</Typography>
                                </Box>
                                <Box sx={{ px: 2, pt: 1.5, pb: 1, flexShrink: 0 }}>
                                    <SearchInput placeholder="Search by name or @user" value={peopleSearch} onChange={(e) => setPeopleSearch(e.target.value)} onSearch={() => { onPeopleSearch(); setMobilePeopleFilterOpen(false); }} onClear={() => { setPeopleSearch(''); setPeopleSearchApplied(''); fetchPeople({ search: '' }); }} inputProps={{ autoFocus: true }} />
                                </Box>
                                <Box sx={{ flex: 1, overflow: 'auto', px: 2, pt: 1, pb: 2 }}>
                                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.25 }}>
                                        <FormControlLabel control={<Checkbox checked={peopleOutOfState} onChange={(e) => { const on = e.target.checked; setPeopleOutOfState(on); if (on) { setPeopleCounty(''); setPeopleCity(''); } else { setPeopleOosCountry(''); setPeopleOosState(''); } }} size="small" icon={<FlightTakeoffIcon sx={{ fontSize: 20, color: 'text.disabled' }} />} checkedIcon={<FlightTakeoffIcon sx={{ fontSize: 20 }} />} sx={{ p: 0.5 }} />} label={<Typography variant="caption" sx={{ fontWeight: 700, lineHeight: 1 }}>Out of State</Typography>} sx={{ mr: 0, ml: 0 }} />
                                        {!peopleOutOfState && (
                                            <CityCountySelect county={peopleCounty || ALL_COUNTIES_LABEL} setCounty={(v) => { setPeopleCity(''); setPeopleCounty(v === ALL_COUNTIES_LABEL ? '' : (v || '')); }} city={peopleCity || ALL_CITIES_LABEL} setCity={(v) => setPeopleCity(v === ALL_CITIES_LABEL ? '' : (v || ''))} onCityCountyChange={({ city: c, county: co }) => { setPeopleCity(c === ALL_CITIES_LABEL ? '' : c); setPeopleCounty(co === ALL_COUNTIES_LABEL ? '' : co); }} allCountyValue={ALL_COUNTIES_LABEL} allCityValue={ALL_CITIES_LABEL} countyCounts={peopleLocationCounts?.counties || null} cityCounts={peopleLocationCounts?.cities || null} emptyCountyLabel="County" emptyCityLabel="City" selectSx={CREAM_INPUT_SX} filterMode />
                                        )}
                                    </Box>
                                </Box>
                                <Box sx={(t) => ({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1.5, px: 2, py: 1.5, borderTop: '1px solid', borderColor: 'divider', bgcolor: t.palette.background.paper, flexShrink: 0 })}>
                                    <Button onClick={() => { onPeopleClear(); setPeopleOutOfState(false); setPeopleOosCountry(''); setPeopleOosState(''); }} sx={{ borderRadius: 999, textTransform: 'none', fontWeight: 800, color: 'text.secondary', px: 2 }}>Reset</Button>
                                    <Button variant="contained" onClick={() => { onPeopleSearch(); setMobilePeopleFilterOpen(false); }} sx={(t) => ({ borderRadius: 999, textTransform: 'none', fontWeight: 900, px: 3, height: 42, bgcolor: t.palette.primary.main, color: t.palette.common.white, boxShadow: 'none', '&:hover': { bgcolor: alpha(t.palette.primary.main, 0.92), boxShadow: 'none' } })}>Show Results</Button>
                                </Box>
                            </Drawer>
                        )}

                        {/* ═══ Mobile Following/Followers filter drawer ═══ */}
                        {isMobile && (
                            <Drawer
                                anchor="bottom"
                                open={mobileFollowFilterOpen}
                                onClose={() => setMobileFollowFilterOpen(false)}
                                PaperProps={{ sx: { height: '100vh', borderRadius: 0, overflow: 'hidden', bottom: 0, display: 'flex', flexDirection: 'column' } }}
                                ModalProps={{ keepMounted: true }}
                                slotProps={{ backdrop: { sx: { bottom: 0 } } }}
                                sx={{ zIndex: (t) => t.zIndex.drawer + 2 }}
                            >
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 2, py: 1.5, ...topInsetSx({ basePadding: 12 }), borderBottom: '1px solid', borderColor: 'divider', flexShrink: 0 }}>
                                    <IconButton onClick={() => setMobileFollowFilterOpen(false)} size="small" sx={{ width: 36, height: 36 }}><ArrowBackRoundedIcon sx={{ fontSize: 22 }} /></IconButton>
                                    <Typography sx={{ fontWeight: 900, fontSize: 16, flex: 1 }}>Search & Filter</Typography>
                                </Box>
                                <Box sx={{ px: 2, pt: 1.5, pb: 1, flexShrink: 0 }}>
                                    <SearchInput placeholder={tab === TAB.FOLLOWING ? 'Search following…' : 'Search followers…'} value={tab === TAB.FOLLOWING ? followingSearch : followersSearch} onChange={(e) => { if (tab === TAB.FOLLOWING) setFollowingSearch(e.target.value); else setFollowersSearch(e.target.value); }} onSearch={() => { if (tab === TAB.FOLLOWING) applyFollowingSearch(); else applyFollowersSearch(); setMobileFollowFilterOpen(false); }} onClear={() => { if (tab === TAB.FOLLOWING) { setFollowingSearch(''); setFollowingSearchApplied(''); } else { setFollowersSearch(''); setFollowersSearchApplied(''); } }} inputProps={{ autoFocus: true }} />
                                </Box>
                                <Box sx={{ flex: 1, overflow: 'auto', px: 2, pt: 1, pb: 2 }}>
                                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                                        {/* Account type chips */}
                                        <Box>
                                            <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary', mb: 0.75, display: 'block' }}>Account Type</Typography>
                                            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75 }}>
                                                {ACCOUNT_TYPE_OPTIONS.filter((o) => o.value).map((o) => {
                                                    const currentType = tab === TAB.FOLLOWING ? followingAccountType : followersAccountType;
                                                    return (<Chip key={o.value} label={o.label} size="small" variant={currentType === o.value ? 'filled' : 'outlined'} color={currentType === o.value ? 'primary' : 'default'} onClick={() => { if (tab === TAB.FOLLOWING) setFollowingAccountType((prev) => prev === o.value ? '' : o.value); else setFollowersAccountType((prev) => prev === o.value ? '' : o.value); }} sx={{ fontWeight: 700, cursor: 'pointer', fontSize: '0.75rem' }} />);
                                                })}
                                            </Box>
                                        </Box>
                                        {/* County/City */}
                                        <Box>
                                            <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary', mb: 0.75, display: 'block' }}>Location</Typography>
                                            <Box sx={{ display: 'grid', gap: 1, gridTemplateColumns: '1fr 1fr' }}>
                                                <Autocomplete size="small" options={[ALL_COUNTIES_LABEL, ...(Array.isArray(peopleCounties) ? peopleCounties : [])]} value={tab === TAB.FOLLOWING ? (followingCounty || ALL_COUNTIES_LABEL) : (followersCounty || ALL_COUNTIES_LABEL)} onChange={(_, val) => { const str = String(val || '').trim(); if (tab === TAB.FOLLOWING) { setFollowingCity(''); if (!str || str === ALL_COUNTIES_LABEL) setFollowingCounty(''); else setFollowingCounty(str); } else { setFollowersCity(''); if (!str || str === ALL_COUNTIES_LABEL) setFollowersCounty(''); else setFollowersCounty(str); } }} renderInput={(params) => <TextField {...params} label="County" />} clearOnEscape autoHighlight filterSelectedOptions />
                                                <Autocomplete size="small" options={[ALL_CITIES_LABEL, ...(tab === TAB.FOLLOWING ? (Array.isArray(followingCities) ? followingCities : []) : (Array.isArray(followersCities) ? followersCities : []))]} value={tab === TAB.FOLLOWING ? (followingCity || ALL_CITIES_LABEL) : (followersCity || ALL_CITIES_LABEL)} onChange={(_, val) => { const str = String(val || '').trim(); if (tab === TAB.FOLLOWING) { if (!str || str === ALL_CITIES_LABEL) setFollowingCity(''); else setFollowingCity(str); } else { if (!str || str === ALL_CITIES_LABEL) setFollowersCity(''); else setFollowersCity(str); } }} renderInput={(params) => { const county = tab === TAB.FOLLOWING ? followingCounty : followersCounty; return <TextField {...params} label={county ? `City (${county})` : 'City'} />; }} clearOnEscape autoHighlight filterSelectedOptions />
                                            </Box>
                                        </Box>
                                        {/* Out of state */}
                                        <FormControlLabel control={<Checkbox checked={tab === TAB.FOLLOWING ? followingOutOfState : followersOutOfState} onChange={(e) => { if (tab === TAB.FOLLOWING) setFollowingOutOfState(e.target.checked); else setFollowersOutOfState(e.target.checked); }} size="small" icon={<FlightTakeoffIcon sx={{ fontSize: 20, color: 'text.disabled' }} />} checkedIcon={<FlightTakeoffIcon sx={{ fontSize: 20 }} />} sx={{ p: 0.5 }} />} label={<Typography variant="caption" sx={{ fontWeight: 700, lineHeight: 1 }}>Out of State</Typography>} sx={{ mr: 0, ml: 0 }} />
                                    </Box>
                                </Box>
                                <Box sx={(t) => ({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1.5, px: 2, py: 1.5, borderTop: '1px solid', borderColor: 'divider', bgcolor: t.palette.background.paper, flexShrink: 0 })}>
                                    <Button onClick={() => { if (tab === TAB.FOLLOWING) { setFollowingSearch(''); setFollowingSearchApplied(''); setFollowingCounty(''); setFollowingCity(''); setFollowingCountyApplied(''); setFollowingCityApplied(''); setFollowingOutOfState(false); setFollowingAccountType(''); } else { setFollowersSearch(''); setFollowersSearchApplied(''); setFollowersCounty(''); setFollowersCity(''); setFollowersCountyApplied(''); setFollowersCityApplied(''); setFollowersOutOfState(false); setFollowersAccountType(''); } }} sx={{ borderRadius: 999, textTransform: 'none', fontWeight: 800, color: 'text.secondary', px: 2 }}>Reset</Button>
                                    <Button variant="contained" onClick={() => { if (tab === TAB.FOLLOWING) applyFollowingSearch(); else applyFollowersSearch(); setMobileFollowFilterOpen(false); }} sx={(t) => ({ borderRadius: 999, textTransform: 'none', fontWeight: 900, px: 3, height: 42, bgcolor: t.palette.primary.main, color: t.palette.common.white, boxShadow: 'none', '&:hover': { bgcolor: alpha(t.palette.primary.main, 0.92), boxShadow: 'none' } })}>Show Results</Button>
                                </Box>
                            </Drawer>
                        )}
                        ...
                        {/* Follow request snackbar */}
                        <SuccessSnackbar {...requestSnackProps} />

                        {/* Report confirmation dialog — matches MusicPostDetailPanel / PostPage style */}
                        <Dialog
